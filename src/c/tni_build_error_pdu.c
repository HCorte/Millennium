...
/*                                                                            */
/*====[TNI_BUILD_ERROR_PDU.C]=================================================*/
/*                                                                            */
/* Build_Error_Pdu (unsigned short conn,                                      */
/*                  struct TNI_PARAM_PAIR *params)                            */
/*                                                                            */
/* Purpose: This function constructs an mbuf chain containing a single        */
/*          TNI Error PDU.  Two separate mbufs are constructed and chained    */
/*          together to form a single chain.  The first mbuf created          */
/*          contains the variable part of the PDU based upon the input        */
/*          parameters.  The second mbuf created contains the fixed header    */
/*          part of the Error PDU.  The mbuf containing the variable part of  */
/*          the PDU is then appended to the mbuf containing the fixed part of */
/*          the PDU to form the returned mbuf chain.                          */
/*                                                                            */
/* Input Arguments:                                                           */
...
/* Determine the maximum size of the fixed and variable ERROR PDU header so   */
/* we can allocate an mbuf of the correct size.  The returned value is larger */
/* than we need for the variable part only, but it's close enough             */

   err_string = null_err_string;

   mlen = Calc_Pdu_Hdr_Size(ERROR_PDU);

   if ((var_hdr_mbuf = mb_alloc(mlen)) == NULL) {

      sprintf(err_string.par1,"variable data");
      sprintf(err_string.par2,"%d",mlen);
      sprintf(err_string.par3,"Build_Error_Pdu");

      output_err("Build_Error_Pdu",
                 MI_TNI_MB_ALLOC,
                 MX_ERR_LVL_ERROR,
                 err_string);
...
/* Build ERROR PDU variable header first to make the total PDU length easier  */
/* to calcuate for the ERROR PDU fixed header                                 */
...
/* Determine the maximum size of the fixed and variable ERROR PDU header so   */
/* we can allocate an mbuf of the correct size.  The returned value is larger */
/* than we need for the fixed part only, but it's close enough                */
...
/* the default verson                                                         */
   ...
   fixed_buf->pdu_type = ERROR_PDU;             /* Error PDU                  */

...
/*                                                                            */
/* Copyright 2006 GTECH Corporation. All rights reserved.                     */
/*                                                                            */
/*====[TNI_BUILD_SERVER_REQUEST_PDU.C]========================================*/
/*                                                                            */
/* Build_Server_Request_Pdu (unsigned short conn,                             */
/*                           struct TNI_PARAM_PAIR *params)                   */
/*                                                                            */
/* Purpose: This function constructs an mbuf chain containing a single        */
/*          Server Request PDU.  Two separate mbufs are constructed and       */
...
/*          of the ServerTNI Request PDU.  The mbuf containing the variable   */
...
/* is larger than we need for the variable part only, but it's close enough.  */
   ...
   mlen = Calc_Pdu_Hdr_Size(SERVER_REQUEST_PDU);
...
/* the default verson                                                         */
   ...
   fixed_buf->pdu_type = SERVER_REQUEST_PDU;
...
/* Now append the 2 mbuf chains - fixed header and variable header            */
   ...
   {
       fprintf(tnicon_p->dbg_p,"\nBuilt server parameter request PDU");
   }

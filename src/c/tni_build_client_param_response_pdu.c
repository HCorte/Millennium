...
/*                                                                            */
/*====[TNI_BUILD_CLIENT_PARAM_RESPONSE_PDU.C]=================================*/
/*                                                                            */
/* Build_Client_Param_Response_Pdu  (unsigned short conn,                     */
/*                                   struct TNI_PARAM_PAIR *params)           */
/*                                                                            */
/* Purpose: This function constructs an mbuf chain containing a single        */
/*          TNI Client Parameter Response PDU.  Two separate mbufs are        */
...
/*          fixed header of the Client Parameter Response PDU.  The mbuf      */
...
struct mbuf *fixed_hdr_mbuf;             /* pointer to mbuf containing fixed  */
                                         /* header part of Client Parameter   */
                                         /* Response  PDU                     */
struct mbuf *var_hdr_mbuf;               /* pointer to mbuf containing        */
                                         /* variable part of Client Parameter */
                                         /* Response  PDU                     */
...
/* Determine the maximum size of the fixed and variable CLIENT RESPONSE PDU   */
...
/* is larger than we need for the variable part only, but it's close enough.  */
   ...
   mlen = Calc_Pdu_Hdr_Size(CLIENT_RESPONSE_PDU);

   if ((var_hdr_mbuf = mb_alloc(mlen)) == NULL) {

      sprintf(err_string.par1,"variable data");
      sprintf(err_string.par2,"%d",mlen);
      sprintf(err_string.par3,"Build_Client_Param_Response_Pdu");

      output_err("Build_Client_Param_Response_Pdu",
                 MI_TNI_MB_ALLOC,
                 MX_ERR_LVL_ERROR,
                 err_string);
...
/* Build CLIENT RESPONSE PDU variable header first to make the total PDU length*/
/* easier to calculate for the CLIENT RESPONSE PDU fixed header                */
...
/* Build CLIENT RESPONSE PDU fixed header                                     */
...
/* the default verson                                                         */
   ...
   fixed_buf->pdu_type = CLIENT_RESPONSE_PDU;   /* Client Response PDU        */
...
/* Now append the 2 mbuf chains - fixed header and variable header            */
   ...
   {
       fprintf(tnicon_p->dbg_p,"\nBuilt client parameter response PDU");
   }

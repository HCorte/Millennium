...
#include "p_return.h"
#include "p_symbol.h"
/* cc /stand=vaxc/debug/noopt/lis/include=dmq$user: MESSAGEQ.C 
...
{
   u_char Mess_To_EuroMil[1024];
   int Mess_To_Len;
}To_EuroMil;
struct MQ_Struct_Get
{
   u_char Mess_From_EuroMil[1024];
   int Mess_From_len;
}From_EuroMil;
...
/*   memset(msg_area,0,MAX_MESSAGE_SIZE);
   ...
   for (j=0;j<To_EuroMil.Mess_To_Len;j++) 
      sprintf(msg_area,"%s%d",msg_area,To_EuroMil.Mess_To_EuroMil[j]);

   for (j=0;j<To_EuroMil.Mess_To_Len;j++) 
       msg_area = (char*) To_EuroMil.Mess_To_EuroMil[j];

   msg_area = (char*) To_EuroMil.Mess_To_EuroMil;  
   /*msg_area = (u_char*) To_EuroMil.Mess_To_EuroMil;
   
   messagSent = (char *) malloc(sizeof(To_EuroMil.Mess_To_Len));
   
   strncpy(msg_area,messagSent,To_EuroMil.Mess_To_Len);
   
   messagSent[To_EuroMil.Mess_To_Len] = '\0';*/
   ...
   dmq_status = pams_put_msg(
                     To_EuroMil.Mess_To_EuroMil,
                     &priority,
                     &q_addrPut,        /* passed in */
                     &msg_class,
                     &msg_type,
                     &delivery,
                     &To_EuroMil.Mess_To_Len,
                     &timeout,
/*                     (int32 *) 0, */  /*  No timeout value       */
                     &put_psb,
                     &uma,
...
void MessQ_get(int *status)
{
   ...
   dmq_status = pams_get_msg(From_EuroMil.Mess_From_EuroMil,
             &priority,
             &q_addr,
             &g_class,
             &g_type,
             &MAX_MESSAGE_SIZE,
             &From_EuroMil.Mess_From_len,
             &sel_filter,
             (struct PSB *) 0,

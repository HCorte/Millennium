...
int main(int argc, char *argv[])
{
    ...
    if(argc < 2) 
    {
       printf("Usage: %s YYYY-MM-DD [output]\n", argv[0]);
       exit(0);
    }

    strncpy( sdate, argv[1], INPLEN); 
    sdate[4] = 0;
    sdate[7] = 0;
    sdate[10] = 0;

    date.year  = atoi(sdate) % 100;
    date.month = atoi(sdate+5);
    date.day   = atoi(sdate+8);
    
    LBDATE( &date );

    if(argc == 2) {
	printf(output_fmt, date.cdc);
	exit(0);
    }  
    ...
    fprintf(fp,output_fmt, date.cdc);
    fclose(fp);
    exit(0); 
}

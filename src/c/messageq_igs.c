...
#include "p_return.h"
#include "p_symbol.h"
/* cc /stand=vaxc/debug/noopt/lis/include=dmq$user: MESSAGEQ_IGS.C
...
{
   u_char Mess_To_IGS[1024];
   int Mess_To_Len;
}To_IGS;
struct MQ_Struct_Get
{
   u_char Mess_From_IGS[1024];
   int Mess_From_len;
}From_IGS;
...
void MessQ_get(int *status)
{
   ...
   dmq_status = pams_get_msg(From_IGS.Mess_From_IGS,
             &priority,
             &q_addr,
             &g_class,
             &g_type,
             &MAX_MESSAGE_SIZE,
             &From_IGS.Mess_From_len,
             &sel_filter,
             (struct PSB *) 0,

C
C SUBROUTINE X2RCLMOD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2RCLMOD.FOV                                 $
C  $Date::   17 Apr 1996 16:28:44                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2rclmod.for **
C
C X2RCLMOD.FOR
C
...
C
C This program will modify a Report Class record.
C
C Calling sequence:
C
C     CALL X2RCLMOD(REC)
...
C
	...
	CHARACTER   CHRSTR(16)*1            !Output string
...
C
	...
	PRTFLG=.FALSE.
	CALL CLRSCR(5)
	CALL FASTMOV(X2XRCL_REC,DMYBUF,X2XRCL_SECT*64)
	WRITE(5,9000)
	WRITE (PROMPT,9020) X2XRCL_ENTRIES-1
C
...
C
	...
	CALL BDATE(DATBUF)
C
C DISPLAY ALL REPORT CLASS INFORMATION PRINTING THREE VARIABLES
C PER LINE.
C
50	CONTINUE
	LINES=X2XRCL_ENTRIES/2
	DO 100 I=1,LINES
	  J=(I-1)*2+1
	  WRITE(5,9010) J,X2XRCL_FIELD(J),
     *	                   X2XRCL_REC(X2XRCL_INDEX(J)),
     *	               J+1,X2XRCL_FIELD(J+1),
     *	                   X2XRCL_REC(X2XRCL_INDEX(J+1))
	  IF(PRTFLG)
     *	    WRITE(6,9010) J,X2XRCL_FIELD(J),
     *	                    X2XRCL_REC(X2XRCL_INDEX(J)),
     *	                J+1,X2XRCL_FIELD(J+1),
     *	                    X2XRCL_REC(X2XRCL_INDEX(J+1))
100	CONTINUE
...
C
	WRITE(5,*)
        WRITE (5,9022)                                                     !V03
	CALL INPNUM(PROMPT,FIELD,2,X2XRCL_ENTRIES-1,EXT)
	IF(EXT.EQ.-2) THEN
	  PRTFLG=.TRUE.
	  OPEN(6, FILE='X2RCLMOD.REP', STATUS='NEW', DISP='PRINT/DELETE')
	  IF(ST.NE.0) THEN
...
C
110	CONTINUE
	BOT=X2XRCL_RANGE(1,FIELD)
	TOP=X2XRCL_RANGE(2,FIELD)
	IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	  ENDOFF=0
	  IF(X2XRCL_VALUE(1,FIELD).NE.-1) THEN
	    DO 112 I=1,15
	      IF(X2XRCL_VALUE(I,FIELD).NE.-1) THEN
	        ENDOFF=I
	      ENDIF
112	    CONTINUE
	    WRITE(5,9025) (X2XRCL_VALUE(I,FIELD),I=1,ENDOFF)
	  ENDIF
C
C PROMPT FOR INPUT DATA.
C
	  WRITE (PROMPT2,9030) X2XRCL_FIELD(FIELD),BOT,TOP
...
C
	  IF(X2XRCL_VALUE(1,FIELD).NE.-1) THEN
	    VALID=.FALSE.
	    DO 120 J=1,15
	      IF(TEMP.EQ.X2XRCL_VALUE(J,FIELD)) VALID=.TRUE.
120	    CONTINUE
	    IF(.NOT.VALID) THEN
	      WRITE(5,9070) CHAR(7)
	      GOTO 110
	    ENDIF
	  ENDIF
	  X2XRCL_REC(X2XRCL_INDEX(FIELD))=TEMP
C
C BCD FIELD.
C
	ELSE IF(BOT.EQ.-2) THEN
	  WRITE (PROMPT2,9032) X2XRCL_FIELD(FIELD), TOP
	  CALL WIMG(5,PROMPT2)
	  READ(5,9060) CHRSTR
	  ...
	  ENDIF
C
	  CALL ATOH(CHRSTR,1,TOP,X2XRCL_REC(X2XRCL_INDEX(FIELD)),ERR)
	  IF(ERR.NE.0) THEN
	    WRITE(5,9080) CHAR(7)
	    GOTO 110
	  ENDIF
...
C
	X2XRCL_UPDATE=DATBUF(VCDC)
	CALL X2BSET(X2XRCL_BITMAP,FIELD,XRCL,REC)			    !V02
	UPDATE=.TRUE.
	CALL CLRSCR(5)
	WRITE(5,9000)
	GOTO 50
...
C
	IF(UPDATE) THEN
	  CALL WRITEW(X2XRCL_FDB,REC,X2XRCL_REC,ST)
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XRCL),'WRITEW',ST,REC)
	    CALL GPAUSE
	  ENDIF
...
C
	ELSE
	  CALL WRITEW(X2XRCL_FDB,REC,DMYBUF,ST)
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XRCL),'WRITEW',ST,REC)
	    CALL GPAUSE
	  ENDIF

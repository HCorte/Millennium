...
C
	BSDATE(VCDC) = DKKBSD
	ESDATE(VCDC) = DKKESD
	DRDATE(VCDC) = DKKDAT(CURDRW)
	CALL LCDATE(BSDATE)
	CALL LCDATE(ESDATE)
	CALL LCDATE(DRDATE)
	WRITE(REPLU, 9003) DRAW, (BSDATE(K),K=9,13),
     *                           (ESDATE(K),K=9,13), DISTIM(DKKTIM),
     *                           (DRDATE(K),K=9,13)
	TOTSAL = 0
	DO I = 1, KIGENT
	  DO K = 1, MAXGAM
	    TOTSAL = TOTSAL + DKKSAL(I,K)
          ENDDO
	ENDDO
	WRITE(REPLU, 9004) CMONY(TOTSAL,13,BETUNIT)
	WRITE(REPLU, 9005) DKKWIN
C
	CALL USRCLOS1(REPLU)
	COPY=0
...
8002	FORMAT('JO',I1,'RESULTS','.REP')
C
9000	FORMAT(1X,131('='))
9001    FORMAT(/,1X,'Draw',32X,'Start Date',4X,'End Date',5X,'Time',8X,'Draw Date')
9002	FORMAT(1X,131('-'))
9003	FORMAT(/,I4,33X 5A2,4X,5A2,3X,A8,4X,5A2)
9004    FORMAT(/,1X,'Sales:',23X,A13)
9005	FORMAT(/,1X,'Winning Number: ',I7.7)
C
	RETURN
	END

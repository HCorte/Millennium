C
C SUBROUTINE X2TERMOD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2TERMOD.FOV                                 $
C  $Date::   17 Apr 1996 16:38:22                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - x2termod.for;1 **
C
C X2TERMOD.FOR
C
C V05 31-JUL-95 DAS Added call to x2cnvdrp
...
C
C This program will modify a terminal on a port.
C
C Calling sequence:
C
C     CALL X2TERMOD(REC)
...
C
	...
	WRITE(5,9000)
	WRITE (PROMPT,9020) X2XTER_ENTRIES-1
C
...
C
     ...
     *	                    X2XTER_DROP
	    IF(PRTFLG)
     *	      WRITE(6,9040) 3,X2XTER_FIELD(3),X2XTER_REC(3),
     *	                    4,X2XTER_FIELD(4),
     *	                      X2XTER_DROP
	  ELSE
	    J=OFFSET
...
C
	WRITE(5,*)
        WRITE (5,9022)                                                     !V03
	CALL INPNUM(PROMPT,FIELD,2,X2XTER_ENTRIES-1,EXT)
	IF(EXT.EQ.-2) THEN
	  PRTFLG=.TRUE.
	  OPEN(6, FILE='X2TERMOD.REP', STATUS='NEW', DISP='PRINT/DELETE')
	  IF(ST.NE.0) THEN
...
C
110	CONTINUE
	BOT=X2XTER_RANGE(1,FIELD)
	TOP=X2XTER_RANGE(2,FIELD)
	IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	  ENDOFF=0
	  IF(X2XTER_VALUE(1,FIELD).NE.-1) THEN
	    DO 112 I=1,15
	      IF(X2XTER_VALUE(I,FIELD).NE.-1) THEN
	        ENDOFF=I
	      ENDIF
112	    CONTINUE
	    WRITE(5,9025) (X2XTER_VALUE(I,FIELD),I=1,ENDOFF)
	  ENDIF
C
C PROMPT FOR INPUT DATA.
C
	  WRITE (PROMPT2,9030) X2XTER_FIELD(FIELD),BOT,TOP
...
C
	  IF(X2XTER_VALUE(1,FIELD).NE.-1) THEN
	    VALID=.FALSE.
	    DO 120 J=1,15
	      IF(TEMP.EQ.X2XTER_VALUE(J,FIELD)) VALID=.TRUE.
120	    CONTINUE
	    IF(.NOT.VALID) THEN
	      WRITE(5,9070) CHAR(7)
	      GOTO 110
	    ENDIF
	  ENDIF
	  X2XTER_REC(X2XTER_INDEX(FIELD))=TEMP
C
C CHARACTER FIELD.
C
	ELSE IF(BOT.EQ.-1) THEN
	  WRITE (PROMPT2,9032) X2XTER_FIELD(FIELD), TOP
	  CALL WIMG(5,PROMPT2)
	  READ(5,9060) CHRSTR
C**	  IF(CHRSTR(1).EQ.'E' .OR. CHRSTR(1).EQ.'e') THEN
C**	    CALL CLRSCR(5)
C**	    WRITE(5,9000)
C**	    GOTO 50
C**	  ENDIF
...
C
	...
	CALL X2BSET(X2XTER_BITMAP,FIELD,XTER,REC)			    !V02
	UPDATE=.TRUE.
	CALL CLRSCR(5)
	WRITE(5,9000)
	GOTO 50
...
9000	FORMAT(//,T26,'GTECH Distributed Network',/,
     *	          T24,'Modify Terminal Configuration',//)

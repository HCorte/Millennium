...
C
C  $Logfile::   GXAFXT:[GOLS]X2NPCMEN.FOV                                 $
C  $Date::   17 Apr 1996 16:25:44                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - x2npcmen.for;1 **
C
C X2NPCMEN.FOR
C
C V01 01-AUG-90 XXX RELEASED FOR VAX
C
C This program provides an interface into the Network Port
...
C
C OPEN THE HOST GLOBAL ADDRESS FILE.
C
...
C
	IF(OPT.NE.4) THEN
	  PROMPT=NULL
	  WRITE (PROMPT,9020)
	  CALL INPNUM(PROMPT,REC,X2XNPC_RANGE(1,1),
     *	              X2XNPC_RANGE(2,1),EXT)
...
C
	...
	IF(OPT.EQ.1) THEN
104	  CONTINUE
	  CALL READL(X2XNPC_FDB,REC,X2XNPC_REC,ST,LOCK)
	  IF(LOCK.NE.0) THEN
	    LOCKCNT=LOCKCNT+1
	    CALL XWAIT(1,2,ST)
	  ...
	  ENDIF
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XNPC),'READL',ST,1)
	    CALL GPAUSE
	  ENDIF
	  IF(X2XNPC_REC(1).GT.0) THEN
	    WRITE(5,9032) REC,CHAR(7)
	    CALL X2UNLOCK(XNPC,X2XNPC_FDB,REC,X2XNPC_REC)
	    CALL XWAIT(2,2,ST)
	    GOTO 100
	  ENDIF
	  CALL X2NPCADD(REC)
...
9000	FORMAT(////,T26,'GTECH Distributed Network',/,
     *	          T25,'Network Port Configuration',//,
     *	          T20,'  1. Add a Network Port',/,
     *	          T20,'  2. Modify a Network Port',/,
     *	          T20,'  3. Delete a Network Port',/,
     *	          T20,'  4. Network Port Listing',/,
     *	          T20,'  E. Exit',//)
9010	FORMAT(25(' '),'Enter option [1-4] ')
9020	FORMAT(25(' '),'Enter network port  ')

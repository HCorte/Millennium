C
C SUBROUTINE FRMPCONF
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]FRMPCONF.FOV                                 $
C  $Date::   17 Apr 1996 13:13:50                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - frmpconf.for;1 **
C
C FRMPCONF.FOR
C
C V01 28-NOV-90 XXX RELEASED FOR VAX
C
C CALL FRMPCONF(SSAP,DSAP,BUF)
C
C IN:
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE FRMPCONF(SSAP,DSAP,BUF)
	IMPLICIT NONE
C
...
C
	IF(LANTEST.NE.0) THEN
	   CALL OPS('**** POLL CONF ****',CONN,LAN)
	   IF(SSAP.LE.0.OR.SSAP.GT.MAXSAP.OR.
     *	      DSAP.LE.0.OR.DSAP.GT.MAXSAP) THEN
	      TYPE*,'**** FRMPCONF:... ILLEGAL SAP ****[',SSAP,DSAP,']'
	      GOTO 200
	   ENDIF
	   IF(CONN.LE.0.OR.CONN.GT.MAXCON) THEN
            TYPE*,'**** FRMPCONF:... ILLEGAL CONNECTION ****[',CONN,']'
	      GOTO 200
	   ENDIF
	   IF(BUF.LE.0.OR.BUF.GT.LANBNUM) THEN
           TYPE*,'**** FRMPCONF:... ILLEGAL BUF ****[',BUF,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	   IF(LAN.LE.0.OR.LAN.GT.MAXLAN) THEN
           TYPE*,'**** FRMPCONF:... ILLEGAL LAN ****[',LAN,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	ENDIF
C

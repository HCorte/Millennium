C SUBROUTINE TRWIN_WININT
C
C V08 03-JUL-2000 UXN LTRLAT added
...
C=======OPTIONS /CHECK=NOOVERFLOW
	...
	SUBROUTINE TRWIN_WININT(FILES,FILCNT)
...
C-------------------- Start of code -----------------------------------
	...
	CALL FASTSET(-1000,LTRDRW,NUMTRP)

	FILCNT = 0
	WVOL = P(ODD_DRWPCK)

	DO 100 GIND = 1,NUMTRP
     ...
     *            (DRWSTS(MLWININD,GNUM).NE.WINPRV)) DRAW=0
	   ...
	   WRITE(6,930) IAM(),GTNAMES(TTRP),GIND,DRAW
	   ...
	   CALL LOGGAM(TTRP,GIND,DTRREC,LTRSTS)
	   IF (LTRSTS(GIND) .EQ. GAMCAN .OR.
     *         LTRSTS(GIND) .EQ. GAMREF) THEN
	      TRREFUND(GIND) = .TRUE.
	   ELSE
	      DO K=1,3	
	         DO I=1,MAXTRPRW
		   IF (LTRSTA(I,K,GIND) .EQ. GAMCAN) THEN
		      TRREFUND(GIND) = .TRUE.
		      WRITE(6,910) IAM(),GTNAMES(TTRP),GIND,K,I
		   ENDIF
		 ENDDO
	      ENDDO	    
	   END IF

	   IF ( LTRSTS(GIND) .LT. GAMENV .OR.
     *	        LTRSTS(GIND) .EQ. GFINAL .OR.
     *	        LTRSTS(GIND) .EQ. GAMREF) THEN
	     WRITE(6,920) IAM(),GTNAMES(TTRP),GIND,LTRSTS(GIND),GAMENV
	     CALL GPAUSE
 	   END IF
 
           IF(LTRSTS(GIND).EQ.GAMREF) LTRSTS(GIND) = GAMCAN
	
	   IF(LTRLAT(LATCDC,GIND).GT.0) TRREFUND(GIND) = .TRUE.
	   LTRLAT(LATCNT,GIND) = 0
	   LTRLAT(LATAMT,GIND) = 0
	   ...
	   LTRTAX(GIND) = 0
           ...
           LTRREF(GIND)   = 0
           LTRWON(GIND)   = 0
           CALL FASTSET(0,LTRWCP(1,GIND),MAXTRPTI)
           CALL FASTSET(0,LTRWBT(1,1,GIND),NUMTOT*MAXTRPTI)
           CALL FASTSET(0,LTRWRA(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LTRWPR(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LTRWRO(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LTRWPA(1,1,GIND),NUMTOT*2)

           CALL FASTSET(0,LTRWPO(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LTRHST(1,GIND),MAXTRPTI)
	   IF(WVOL.EQ.0) THEN
	   ...
	   DO 90 CDC = LTRBSD(GIND),LTRESD(GIND)
...
910	FORMAT(1X,A,1X,A8,I1,' refund scan for event ',I1,' row ',I2)

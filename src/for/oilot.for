C  GXSRC:OILOT.FOR
C
C V08 09-OCT-2000 UXN AlphaIPS release.
C V07 05-DEC-1996 HXK Updated for Finland IPS
C V06 01-FEB-1995 JJOLY UPDATED TO MATCH NEW ERROR MESSAGE MAPPING
C V05 07-JUN-1993 MCM RELEASED FOR GEORGIA
...
C
C SUBROUTINE TO BUILD INSTANT LOT OUTPUT MESSAGES.
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE OILOT(TRABUF,OUTTAB,OUTLEN)
	IMPLICIT NONE
C
...
C INSTANT CLASS
C
	OUTTAB(IND) = TRABUF(TLCLS)
	IND=IND+1
C
C AMOUNT SOLD
C
        IF(TRABUF(TLCLS).NE.6) THEN
C
	   CALL HOST_TO_TERM(OUTTAB(IND), TRABUF(TLAMT), 4)
	   IND=IND+4
C
C COMMISION AMT
C
	   CALL HOST_TO_TERM(OUTTAB(IND), TRABUF(TLCOM), 4)
	   IND=IND+4
C
        ENDIF
	OUTLEN=IND-1

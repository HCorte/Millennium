C
C SUBROUTINE X2RCLDEL
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2RCLDEL.FOV                                 $
C  $Date::   17 Apr 1996 16:28:24                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2rcldel.for;1 **
C
C X2RCLDEL.FOR
C
...
C
C This program will mark a record in the Report class file
...
C
	IF(ANS.EQ.1) THEN
	  ...
	  X2XRCL_REC(1)=0
CV02	  CALL BSET(X2XRCL_BITMAP,1)
...
C
	IF(UPDATE) THEN
	  ...
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XRCL),'WRITEW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  WRITE(5,9080) REC

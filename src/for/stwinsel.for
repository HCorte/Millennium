...
C STWINSEL.FOR
C
C SUPER TRIPLE WINNER SELECTION PROGRAM.
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C---- Initialize winner selection common.
     ...
     *            'This program can be run only from *WINTSK or MULTIWIN'
        ...
        IF(STOPMOD.EQ.WINMANUAL) THEN
             CALL STWIN_WININT(CFILES,FILCNT)
        ELSE
             NTSK=INDTSK('STWINTSK')
             CALL STORFIL(NTSK,CFILES,DUMMY,FILCNT,2,0)
        ENDIF
C
	IF(FILCNT.EQ.0) THEN
	  TYPE*,IAM(),' Sorry, no Super Triple winner selection today'
C	  CALL GSTOP(GEXIT_FATAL)
	ENDIF

	CALL STWIN_WINLOD(1,DUMMY)
...
C---- Start draw file scan.
	...
	DO 3000 I = 1,FILCNT
	  CALL STWIN_OPNDRW(FILES(1,I),PTMF)
...
2030	  CONTINUE
	  ...
	  IF (GTYP .NE. TSTR) GOTO 2030

C---- Check if winner.

	  CALL STWIN_POST(TRABUF)

	  CALL STCHKWIN(TRABUF,V4BUF,WIN)

	  IF (WIN .NE. 0) THEN
	    CALL STWIN_WINLOD(2,V4BUF)
	    CALL FASTSET(0,V4BUF,VFLEN*4)
	  END IF

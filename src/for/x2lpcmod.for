C
C SUBROUTINE X2LPCMOD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2LPCMOD.FOV                                 $
C  $Date::   17 Apr 1996 16:23:04                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2lpcmod.for **
C
C X2LPCMOD.FOR
C
...
C
C This program will modify a Local Port record.
C
C Calling sequence:
C
C     CALL X2LPCMOD(REC)
...
C
	...
	PRTFLG=.FALSE.
	CALL CLRSCR(5)
	CALL FASTMOV(X2XLPC_REC,DMYBUF,X2XLPC_SECT*64)
	WRITE(5,9000)
	WRITE (PROMPT,9020) X2XLPC_ENTRIES-1
C
...
C
50	CONTINUE
	LINES=X2XLPC_ENTRIES/2
	DO 100 I=1,LINES
	  OFFSET=(I-1)*2+1
	  J=OFFSET
	  WRITE(5,9010) J,X2XLPC_FIELD(J),
     *	                  X2XLPC_REC(X2XLPC_INDEX(J)),
     *	              J+1,X2XLPC_FIELD(J+1),
     *	                  X2XLPC_REC(X2XLPC_INDEX(J+1))
	  IF(PRTFLG)
     *	    WRITE(6,9010) J,X2XLPC_FIELD(J),
     *	                    X2XLPC_REC(X2XLPC_INDEX(J)),
     *	                J+1,X2XLPC_FIELD(J+1),
     *	                    X2XLPC_REC(X2XLPC_INDEX(J+1))
100	CONTINUE
...
C
	WRITE(5,*)
        WRITE (5,9022)                                                     !V03
	CALL INPNUM(PROMPT,FIELD,2,X2XLPC_ENTRIES-1,EXT)
	IF(EXT.EQ.-2) THEN
	  PRTFLG=.TRUE.
	  OPEN(6, FILE='X2LPCMOD.REP', STATUS='NEW', DISP='PRINT/DELETE')
	  IF(ST.NE.0) THEN
...
C
110	CONTINUE
	BOT=X2XLPC_RANGE(1,FIELD)
	TOP=X2XLPC_RANGE(2,FIELD)
	IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	  ENDOFF=0
	  IF(X2XLPC_VALUE(1,FIELD).NE.-1) THEN
	    DO 112 I=1,15
	      IF(X2XLPC_VALUE(I,FIELD).NE.-1) THEN
	        ENDOFF=I
	      ENDIF
112	    CONTINUE
	    WRITE(5,9025) (X2XLPC_VALUE(I,FIELD),I=1,ENDOFF)
	  ENDIF
C
C PROMPT FOR INPUT DATA.
C
	  WRITE (PROMPT2,9030) X2XLPC_FIELD(FIELD),BOT,TOP
...
C
	  IF(X2XLPC_VALUE(1,FIELD).NE.-1) THEN
	    VALID=.FALSE.
	    DO 120 J=1,15
	      IF(TEMP.EQ.X2XLPC_VALUE(J,FIELD)) VALID=.TRUE.
120	    CONTINUE
	    IF(.NOT.VALID) THEN
	      WRITE(5,9070) CHAR(7)
	      GOTO 110
	    ENDIF
	  ENDIF
	  X2XLPC_REC(X2XLPC_INDEX(FIELD))=TEMP
C
C BCD FIELD.
C
	ELSE IF(BOT.EQ.-2) THEN
	  WRITE (PROMPT2,9032) X2XLPC_FIELD(FIELD), TOP
	  CALL WIMG(5,PROMPT2)
	  READ(5,9060) CHRSTR
	  ...
	  ENDIF
C
	  CALL ATOH(CHRSTR,1,TOP,X2XLPC_REC(X2XLPC_INDEX(FIELD)),ERR)
	  IF(ERR.NE.0) THEN
	    WRITE(5,9080) CHAR(7)
	    GOTO 110
	  ENDIF
...
C
	...
	CALL X2BSET(X2XLPC_BITMAP,FIELD,XLPC,REC)			    !V02
	UPDATE=.TRUE.
	CALL CLRSCR(5)
	WRITE(5,9000)
	GOTO 50
...
C
	IF(UPDATE) THEN
	  CALL WRITEW(X2XLPC_FDB,REC,X2XLPC_REC,ST)
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XLPC),'WRITEW',ST,REC)
	    CALL GPAUSE
	  ENDIF
...
C
	ELSE
	  CALL WRITEW(X2XLPC_FDB,REC,DMYBUF,ST)
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XLPC),'WRITEW',ST,REC)
	    CALL GPAUSE
	  ENDIF

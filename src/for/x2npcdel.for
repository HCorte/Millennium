C
C SUBROUTINE X2NPCDEL
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2NPCDEL.FOV                                 $
C  $Date::   17 Apr 1996 16:25:34                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2npcdel.for **
C
C X2NPCDEL.FOR
C
...
C
C This program will mark a record in the Network Configuration file
...
C
	IF(ANS.EQ.1) THEN
	  ...
	  X2XNPC_REC(1)=0
CV02	  CALL BSET(X2XNPC_BITMAP,1)
...
C
	IF(UPDATE) THEN
	  ...
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XNPC),'WRITEW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  WRITE(5,9080) REC

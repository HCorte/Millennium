...
C
C  $Logfile::   GXAFXT:[GOLS]X2POST.FOV                                   $
C  $Date::   17 Apr 1996 16:26:18                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - x2post.for;1 **
C
C X2POST.FOR
C
C V03 21-OCT-94 GPR TEST THE FIRST BIT OF THE BITMAP NOT THE LAST - Integrate 
C		    UK changes into X2X Baseline
C V02 23-FEB-94 GPR USE I5 FORMAT FOR STATION AND TERMINAL TYPE-OUTS
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2POST(DISLOG,PRTFLG,UPDFLG)
	IMPLICIT NONE
C
...
C
     ...
     *	     ALLREC(X2XFIL_BITMAP(FILIDX)+1).NE.0 .OR.		    !V03
     *	     ALLREC(X2XFIL_BITMAP(FILIDX)+2).NE.0 .OR.		    !V03
     *	     ALLREC(X2XFIL_BITMAP(FILIDX)+3).NE.0) THEN		    !V03
C
C SET FLAG INDICATING WHETHER A MODIFICATION (1), OR AN
C ADDITION(0).
C
	    ADDFLG=1
CV03	    IF(ALLREC(X2XFIL_BITMAP(FILIDX)).EQ.-1  .AND.
...
310	    CONTINUE
	    ...
	    ALLREC(X2XFIL_BITMAP(FILIDX)+2)=0			!V03
	    ALLREC(X2XFIL_BITMAP(FILIDX)+3)=0			!V03
	    IF(BIGREAD) THEN

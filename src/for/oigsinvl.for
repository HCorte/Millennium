...
C --- ---------- ------- ----------------------------------------------
C V01 2014.05.20  SCML   Created - PLACARD Project - Creates invalid 
...
C=======OPTIONS /CHECK=NOOVERFLOW/EXT
        SUBROUTINE OIGSINVL(WRKBUF, TRABUF, MESS, MES_LEN, BUFNUM, TER, ST)
        IMPLICIT NONE
...
C----+------------------------------------------------------------------
        ...
        IF(IGSDEBUG(IA_OUTIGS)) THEN
            CALL OPS('OIGSINVL: ITYPE   ',ITYPE,ITYPE)
            CALL OPS('OIGSINVL: ISUBTYPE',ISUBTYPE,ISUBTYPE)
            CALL OPS('OIGSINVL: OTYPE   ',OTYPE,OTYPE)
            CALL OPS('OIGSINVL: OSUBTYPE',OSUBTYPE,OSUBTYPE)
            CALL OPS('OIGSINVL: TRABUF(TSDT1)',TRABUF(TSDT1),TRABUF(TSDT1))
            CALL OPS('OIGSINVL: TRABUF(TSDT2)',TRABUF(TSDT2),TRABUF(TSDT2))
        ENDIF
        ...
        ELSEIF(ITYPE .EQ. 9) THEN ! BYPASS TYPE = 9 MESSAGES (ERROR MESSAGES)
            BYPASS_ERROR_MSG_GENERATION = .TRUE.
            CALL OPSTXT('INVALID MESSAGE')
            IF(IGSDEBUG(IA_OUTIGS)) THEN
               CALL OPS('OIGSINVL: ITYPE   ',ITYPE,ITYPE)
               CALL OPS('OIGSINVL: ISUBTYPE',ISUBTYPE,ISUBTYPE)
               CALL OPS('OIGSINVL: OTYPE   ',OTYPE,OTYPE)
     ...
     *                    , SUBTYP
     *                    , IGS_MILSYS
     *                    , 'MIL-00001' ! Invalid transaction
C     *                    , 'TransaÁ„o inv·lida.')
     *                    , 'Transa√ß√£o inv√°lida.' ! UTF-8 Encoded

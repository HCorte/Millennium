...
C V03 10-MAY-1999 UXN TRIPLE CHANGED TO TRIO.
...
C TRWINSEL.FOR
C
C TODAY'S TRIO WINNER SELECTION PROGRAM.
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C---- Initialize winner selection common.
     ...
     *            'This program can be run only from *WINTSK or MULTIWIN'
        ...
        IF(STOPMOD.EQ.WINMANUAL) THEN
             CALL TRWIN_WININT(CFILES,FILCNT)
        ELSE
             NTSK=INDTSK('TRWINTSK')
             CALL STORFIL(NTSK,CFILES,DUMMY,FILCNT,2,0)
        ENDIF
C
	IF(FILCNT.EQ.0) THEN
	  TYPE*,IAM(),' Sorry, no Todays Trio winner selection today'
C	  CALL GSTOP(GEXIT_FATAL)
	ENDIF

	CALL TRWIN_WINLOD(1,DUMMY)
...
C---- Start draw file scan.
	...
	DO 3000 I = 1,FILCNT
	  CALL TRWIN_OPNDRW(FILES(1,I),PTMF)
...
2030	  CONTINUE
	  ...
	  IF (GTYP .NE. TTRP) GOTO 2030

C---- Check if winner.

	  CALL TRWIN_POST(TRABUF)

	  CALL TRCHKWIN(TRABUF,V4BUF,WIN)

	  IF (WIN .NE. 0) THEN
	    CALL TRWIN_WINLOD(2,V4BUF)
	    CALL FASTSET(0,V4BUF,VFLEN*4)
	  END IF

...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE GUI_007(OUTBUF,MES_LEN,RET_CODE)
	IMPLICIT NONE
C
...
C
	CALL GUIARG_INIT()
C
C RESULT SET 1 - number of rows
C
	NUM_COLS = 2
	NUM_ROWS = 1
	CALL GUIARG_NEXT_SET(OUTBUF,NUM_COLS)
C	
	CALL GUIARG_BYTE(OUTBUF,DTGMAX)
...
C	
	DO I=1,NUM_ROWS
           CALL GUIARG_CHAR(OUTBUF,DTGNMS(1,1,I),16)
           CALL GUIARG_CHAR(OUTBUF,DTGNMS(1,2,I),16)
	ENDDO

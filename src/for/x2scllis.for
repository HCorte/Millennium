C
C SUBROUTINE X2SCLLIS
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2SCLLIS.FOV                                 $
C  $Date::   17 Apr 1996 16:33:26                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C  Based on Netherlands Bible, 12/92, and Comm 1/93 update
C  DEC Baseline
C
C ** Source - x2scllis.for;5 **
C
C X2SCLLIS.FOR
C
C V02 20-JUL-94 WS MULTINETWORK CHANGES - Integrate UK changes 
C		   into X2X Baseline
C V01 01-AUG-90 XXX RELEASED FOR VAX
C
C This subroutine will display defined station classes
C stored in the X2XSCL file to the screen.  The user
C will have the option of displaying a specific code,
C or to page forward or back.
C
C Calling sequence:
C
C     CALL X2SCLLIS
C
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C This item is the property of GTECH Corporation, Providence, Rhode
C Island, and contains confidential and trade secret information. It
C may not be transferred from the custody or control of GTECH except
C as authorized in writing by an officer of GTECH. Neither this item
C nor the information it contains may be used, transferred,
C reproduced, published, or disclosed, in whole or in part, and
C directly or indirectly, except as expressly authorized by an
C officer of GTECH, pursuant to written agreement.
C
C Copyright 1994 GTECH Corporation. All rights reserved.
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2SCLLIS
	IMPLICIT NONE
C
	INCLUDE 'INCLIB:SYSPARAM.DEF'
	INCLUDE 'INCLIB:SYSEXTRN.DEF'
C
	INCLUDE 'INCLIB:GLOBAL.DEF'
	INCLUDE 'INCLIB:X2XPRM.DEF'
	INCLUDE 'INCLIB:X2XSCL.DEF'
C
	INTEGER*4   SCL,SCLCNT          !Rec #/station count
	INTEGER*4   LASTSCL             !Last station displayed
	INTEGER*4   EOFCNT              !End of file count
	INTEGER*4   ST,K                !Work variables
	INTEGER*4   PREV, LAST_PREV
	CHARACTER   X2FILNAM*20
	CHARACTER   PROMPT*65           !Output prompt
	CHARACTER   OPT*4               !Input option
	LOGICAL     MORE                !OK to display more
C
C DISPLAY THE NEXT SCREEN OF CODES.
C
	SCL=0
	LASTSCL=0
	LAST_PREV=0
	PREV=0
100	CONTINUE
	IF(SCL.LT.0) SCL=0
	EOFCNT=0
	SCLCNT=0
	CALL CLRSCR(5)
	WRITE(5,9080)
	WRITE(5,9090)
C
C READ A STATION FROM THE FILE AND DISPLAY
C IT TO THE SCREEN.
C
200	CONTINUE
	IF(SCLCNT.LT.10 .AND. EOFCNT.LE.100) THEN
250	  SCL=SCL+1
	  CALL READW(X2XSCL_FDB,SCL,X2XSCL_REC,ST)
	  IF(ST.EQ.144) GOTO 300
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XSCL),'READW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  IF(X2XSCL_REC(1).LE.0) THEN
	    EOFCNT=EOFCNT+1
	    IF(EOFCNT.LE.100) GOTO 250
	    GOTO 300
	  ELSE
	    EOFCNT=0
	  ENDIF
C
C DISPLAY THE CODE TO THE SCREEN.
C
	  SCLCNT=SCLCNT+1
	  IF(SCLCNT.EQ.1) THEN
	    LAST_PREV=PREV
	    PREV=SCL
	  ENDIF
	  LASTSCL=SCL
	  WRITE(5,9100) X2XSCL_CLASS,  X2XSCL_DESC,
     *	                X2XSCL_INTIM,  X2XSCL_OUTTIM,
     *	                X2XSCL_RESTIM, X2XSCL_INTTIM,
     *	                X2XSCL_STSTIM, X2XSCL_SUBNETWORK,	!V02
     *                  X2XSCL_BCST_ENABLE1,X2XSCL_BCST_ENABLE2 !V03
	  GOTO 200
	ENDIF
C
C IF NO CODE EXIST, DISPLAY AN APPROPRIATE MESSAGE.
C
300	CONTINUE
	IF(SCLCNT.EQ.0) THEN
	  WRITE(5,9050)
	ENDIF
C
C BUILD THE APPROPRIATE OUTPUT PROMPT.
C
	IF(EOFCNT.LE.100) THEN
	  WRITE (PROMPT,9000)
	  MORE=.TRUE.
	ELSE
	  WRITE (PROMPT,9010)
	  MORE=.FALSE.
	ENDIF
C
C ASK USER WHAT THEY WANT TO DO.
C
	WRITE(5,*)
	WRITE(5,*)
350	CONTINUE
	CALL WIMG(5,PROMPT)
	READ(5,9020) OPT
	IF(OPT(1:1).EQ.'E'.OR.OPT(1:1).EQ.'e') THEN
	  GOTO 8000
	ELSE IF(MORE.AND.(OPT(1:1).EQ.'N'.OR.OPT(1:1).EQ.'n')) THEN
	  SCL=LASTSCL
	  GOTO 100
	ELSE IF(OPT(1:1).EQ.'P'.OR.OPT(1:1).EQ.'p') THEN
	  SCL=LAST_PREV-1
	  GOTO 100
	ELSE IF(OPT(1:1).EQ.'X'.OR.OPT(1:1).EQ.'x') THEN
	  SCL=CTOI(OPT(2:4),K)-1
	  GOTO 100
	ELSE
	  WRITE(5,9040)
	  GOTO 350
	ENDIF
C
C PROGRAM EXIT
C
8000	CONTINUE
	RETURN
C
C     ==================== Format Statements ===================
C
9000  FORMAT(15(' '),'Enter N for next, P for Previous, ',
     *	               'or X# for record')
9010  FORMAT(15(' '),'Enter P for previous or X# for class record ')
9020  FORMAT(A4)
9040  FORMAT(15(' '),'Invalid option input ')
9050  FORMAT(15(' '),'Sorry, no more class records exist ')
9080  FORMAT(//,T26,'GTECH Distributed Network',/,
     *          T27,'Station Class Listing',//)
9090  FORMAT(T7,'Class',T17,'Desc',T27,'Incall',
     *	       T35,'Outcall',T44,'Resp',T51,'Inter',T58,'Stats',
     *	       T65,'Snet',T70,'BSEna',					!V02
     *	       /,T7,68('='))						!V02
9100  FORMAT(T8,I3,T14,A12,T27,I5,T35,I5,T43,I5,T50,I5,T57,I5,T64,I4,
     *       T71,I1,'/',I1)						!V02
      END

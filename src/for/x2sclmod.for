C
C SUBROUTINE X2SCLMOD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2SCLMOD.FOV                                 $
C  $Date::   17 Apr 1996 16:33:40                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - x2sclmod.for;1 **
C
C X2SCLMOD.FOR
C
C V05 22-DEC-94 DAS Fixed back and next command
...
C
C This program will modify a Station Class record.
C
C Calling sequence:
C
C     CALL X2SCLMOD(REC)
...
C
	...
	CALL X2BSET(X2XSCL_BITMAP,FIELD,XSCL,REC)			    !V02
	UPDATE=.TRUE.
	CALL CLRSCR(5)
	WRITE(5,9000) PAGE,MAXPAGE
	GOTO 50
...
C9075    FORMAT(T3,2(I2.2,'.',1X,A15,1X,15A,2X))
9075    FORMAT(T3,I2.2,'.',1X,A15,1X,<LEN1>A,<16-LEN1>X,2X
     *           ,I2.2,'.',1X,A15,1X,<LEN2>A,<16-LEN1>X,2X)

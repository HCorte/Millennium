C
C SUBROUTINE EXTRABUF
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]EXTRABUF.FOV                                 $
C  $Date::   17 Apr 1996 13:07:16                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*
C     SUBROUTINE EXTRABUF(BUF,WAY,ST);  GET BUFFER
C     OUT - BUF - BUFFER #
C           ST  - SAME AS RTL AND RBL
C
C     IN - WAY - CURRENT WAY
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE EXTRABUF(BUF,WAY,ST)
	IMPLICIT NONE
C
        INCLUDE '($SSDEF)'
        INCLUDE '($SYSSRVNAM)'
...
C
	...
	IF (ST.EQ.2) THEN
	   IF (NETCMDFRZ.NE.0.AND.WAY.EQ.RECOVWAY)
     *	   CALL RTL(BUF,RECOVQUE,ST)
	   IF (ST.EQ.2) THEN
	      CALL RTL(BUF,NETEXTRA(1,WAY),ST)
	      IF (ST.EQ.2) RETURN
	   ENDIF
	ENDIF
C

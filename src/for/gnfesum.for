C
C SUBROUTINE GNFESUM
C $Log:   GXAFXT:[GOLS]GNFESUM.FOV  $
C  
C     Rev 1.0   17 Apr 1996 13:26:08   HXK
C  Release of Finland for X.25, Telephone Betting, Instant Pass Thru Phase 1
C  
C     Rev 1.0   21 Jan 1993 16:31:16   DAB
C  Initial Release
C  Based on Netherlands Bible, 12/92, and Comm 1/93 update
C  DEC Baseline
C
C ** Source - gnfesum.for **
C
* GNFESUM.FOR
*
...
* summary messages for GNOS.  The routine will send
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE GNFESUM(MESTYP,SUBTYP,MSGID,TYP,CNT,QUADTA)
	IMPLICIT NONE
*
...
*
	...
	IF(TYP(1).EQ.GNHDRMES_TYP_RANGE) THEN
	  DO 100 OUTSAP=QUADTA(1,1),MIN0(QUADTA(2,1),X2X_SAP)
	    CALL BLDFESUM(OUTSAP,MESTYP,SUBTYP,MSGID,MSGSEQ,MESLEN)
	    IF(MESLEN.LT.0) THEN

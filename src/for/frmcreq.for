C
C SUBROUTINE FRMCREQ
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]FRMCREQ.FOV                                  $
C  $Date::   17 Apr 1996 13:13:26                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - frmcreq.for;1 **
C
C FRMCREQ.FOR
C
C V01 28-NOV-90 XXX RELEASED FOR VAX
C
C CALL FRMCREQ(SSAP,DSAP,BUF)
C
C IN:
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE FRMCREQ(SSAP,DSAP,BUF)
	IMPLICIT NONE
C
...
C
	IF(LANTEST.NE.0) THEN
	   CALL OPS('**** CON REQ ****',CONN,LAN)
	   IF(SSAP.LE.0.OR.SSAP.GT.MAXSAP.OR.
     *	      DSAP.LE.0.OR.DSAP.GT.MAXSAP) THEN
	      TYPE*,'**** FRMCREQ:... ILLEGAL SAP ****[',SSAP,DSAP,']'
	      GOTO 200
	   ENDIF
	   IF(CONN.LE.0.OR.CONN.GT.MAXCON) THEN
            TYPE*,'**** FRMCREQ:... ILLEGAL CONNECTION ****[',CONN,']'
	      GOTO 200
	   ENDIF
	   IF(BUF.LE.0.OR.BUF.GT.LANBNUM) THEN
           TYPE*,'**** FRMCREQ:... ILLEGAL BUF ****[',BUF,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	   IF(LAN.LE.0.OR.LAN.GT.MAXLAN) THEN
           TYPE*,'**** FRMCREQ:... ILLEGAL LAN ****[',LAN,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	ENDIF
C

C
C SUBROUTINE FRMDCONF
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]FRMDCONF.FOV                                 $
C  $Date::   17 Apr 1996 13:13:34                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE FRMDCONF(SSAP,DSAP,BUF)
	IMPLICIT NONE
C
...
C
	IF(LANTEST.NE.0) THEN
	   CALL OPS('**** DIS CONF ****',CONN,LAN)
	   IF(SSAP.LE.0.OR.SSAP.GT.MAXSAP.OR.
     *	      DSAP.LE.0.OR.DSAP.GT.MAXSAP) THEN
	      TYPE*,'**** FRMDCONF:... ILLEGAL SAP ****[',SSAP,DSAP,']'
	      GOTO 200
	   ENDIF
	   IF(CONN.LE.0.OR.CONN.GT.MAXCON) THEN
            TYPE*,'**** FRMDCONF:... ILLEGAL CONNECTION ****[',CONN,']'
	      GOTO 200
	   ENDIF
	   IF(BUF.LE.0.OR.BUF.GT.LANBNUM) THEN
           TYPE*,'**** FRMDCONF:... ILLEGAL BUF ****[',BUF,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	   IF(LAN.LE.0.OR.LAN.GT.MAXLAN) THEN
           TYPE*,'**** FRMDCONF:... ILLEGAL LAN ****[',LAN,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	ENDIF
C

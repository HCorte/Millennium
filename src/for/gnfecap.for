C
C SUBROUTINE GNFECAP
C $Log:   GXAFXT:[GOLS]GNFECAP.FOV  $
C  
C     Rev 1.0   17 Apr 1996 13:25:44   HXK
C  Release of Finland for X.25, Telephone Betting, Instant Pass Thru Phase 1
C  
C     Rev 1.0   21 Jan 1993 16:31:00   DAB
C  Initial Release
C  Based on Netherlands Bible, 12/92, and Comm 1/93 update
C  DEC Baseline
C
C ** Source - gnfecap.for **
C
* GNFECAP.FOR
*
...
*
* This subroutine will build the requested Front end network
* port capacity information messages for GNOS.  The
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE GNFECAP(MESTYP,SUBTYP,MSGID,TYP,CNT,QUADTA)
	IMPLICIT NONE
*
...
*
	...
	IF(TYP(1).EQ.GNHDRMES_TYP_RANGE) THEN
	  CALL FC_RNG_SAP(MESTYP,SUBTYP,MSGID,
     *	                  MSGSEQ,TYP,CNT,QUADTA)
	ELSEIF(TYP(1).EQ.GNHDRMES_TYP_SPECIFIC) THEN
	  CALL FC_SPE_SAP(MESTYP,SUBTYP,MSGID,
     *	                  MSGSEQ,TYP,CNT,QUADTA)
	ELSE
	  CALL FC_ALL_SAP(MESTYP,SUBTYP,MSGID,
     *	                  MSGSEQ,TYP,CNT,QUADTA)
	ENDIF
*

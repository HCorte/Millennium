...
C
        CALL COPYRITE
C
        NEW_FILE = 'FILE:CP1F.NEW'
        OLD_FILE = 'FILE:CP1F.FIL'
C
...
C
	SIZE = (SIZE/10) * 10
C
        MAXREC_CNV = SIZE/(DCPSEC/2)
        FILE_SIZE  = MAXREC_CNV*DCPSEC/2
        TYPE*,IAM(),'Old file size = ', MAXREC_CNV*DCPSEC/2
        TYPE*,IAM(),'New file size = ', MAXREC_CNV*DCPSEC/2
        TYPE*,IAM()
        TYPE*,IAM(),'Number of records to convert >',MAXREC_CNV
        TYPE*,IAM()
        CALL PRMYESNO('Are you sure you want to convert CP%F.FIL ',YESNO)
        IF(YESNO.NE.1) CALL GSTOP(GEXIT_OPABORT)
C
C Create new CP%F.NEW
C	
	DO I=1,NUMCPL
...
900     FORMAT('FILE:CP',I1,'F',A4,T20)
...
C
	...
	INTEGER*4    INP_FDB(7)
	INTEGER*4    OUT_FDB(7)
	INTEGER*4    TMP1(2,MAXCPLTI)
	INTEGER*4    TMP2(2,MAXCPLTI)
	INTEGER*4    TOT_WIN
C
...
C
	...
	DO WHILE(.TRUE.)
	   REC = REC + 1
	   CALL READW(INP_FDB,REC,DCPREC,ST)
	   IF(ST.EQ.144) GOTO 10 ! END-OF-FILE
	   IF(ST.NE.0) CALL FILERR(INP_FILE,2,ST,REC)
C	   
	   CALL FASTMOV(DCPWIN,TMP1,2*MAXCPLTI)
	   CALL FASTMOV(DCPWIN,TMP2,2*MAXCPLTI)
	   CALL FASTSET(0,DCPWIN,2*MAXCPLTI)
	   CALL FASTSET(0,DCPHLD,2*MAXCPLTI)
C
	   TOT_WIN = 0
	   DO 110 I=1,MAXCPLTI
	      IF(TMP1(1,I).LE.0) GOTO 110
	      DO 100 J=1,MAXCPLTI
	         IF(TMP1(2,J).LE.0) GOTO 100
		 TOT_WIN = TOT_WIN + 1
		 DCPWIN(1,TOT_WIN) = TMP1(1,I)
		 DCPWIN(2,TOT_WIN) = TMP1(2,J)
		 DCPHLD(1,TOT_WIN) = TMP2(1,I)
		 DCPHLD(2,TOT_WIN) = TMP2(2,J)
100	      CONTINUE
110	   CONTINUE
	   DCPCMB = TOT_WIN
	   IF(DCPCMB.GT.1) WRITE(6,910) IAM(),DCPDRW,DCPCMB
C
	   CALL WRITEW(OUT_FDB,REC,DCPREC,ST)

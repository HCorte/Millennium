C
C SUBROUTINE X2LPCADD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2LPCADD.FOV                                 $
C  $Date::   17 Apr 1996 16:22:36                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2lpcadd.for **
C
C X2LPCADD.FOR
C
...
C
C This program will add a Local Port Configuration record.
C
C Calling sequence:
C
C     CALL X2LPCADD(REC)
...
C
	...
	CHARACTER   CHRSTR(10)*1            !Output string
...
C
C
	DO 100 I=2,X2XLPC_ENTRIES-1
110	  CONTINUE
	  BOT=X2XLPC_RANGE(1,I)
	  TOP=X2XLPC_RANGE(2,I)
	  IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	    ENDOFF=0
	    IF(X2XLPC_VALUE(1,I).NE.-1) THEN
	      DO 112 J=1,15
	        IF(X2XLPC_VALUE(J,I).NE.-1) THEN
	          ENDOFF=J
	        ENDIF
112	      CONTINUE
	      WRITE(5,9025) (X2XLPC_VALUE(J,I),J=1,ENDOFF)
	    ENDIF
C
C PROMPT FOR INPUT DATA.
C
	    WRITE (PROMPT2,9030) X2XLPC_FIELD(I),BOT,TOP
	    CALL INPNUM(PROMPT2,X2XLPC_REC(X2XLPC_INDEX(I)),
     *	                BOT,TOP,EXT)
	    IF(EXT.EQ.-9) THEN
...
C
	    IF(X2XLPC_VALUE(1,I).NE.-1) THEN
	      VALID=.FALSE.
	      DO 120 J=1,15
	        IF(X2XLPC_REC(X2XLPC_INDEX(I)).EQ.X2XLPC_VALUE(J,I))
     *	          VALID=.TRUE.
120	      CONTINUE
	      IF(.NOT.VALID) THEN
	        WRITE(5,9070) CHAR(7)
...
C
	  ELSE IF(BOT.EQ.-2) THEN
	    WRITE (PROMPT2,9032) X2XLPC_FIELD(I), TOP
	    CALL WIMG(5,PROMPT2)
	    READ(5,9060) CHRSTR
	    CALL ATOH(CHRSTR,1,TOP,X2XLPC_REC(X2XLPC_INDEX(I)),ERR)
	    IF(ERR.NE.0) THEN
	      WRITE(5,9080) CHAR(7)
	      GOTO 110
	    ENDIF
...
C
	...
	X2XLPC_PORT=REC
CV02	X2XLPC_BITMAP=-1
	CALL X2BSET(X2XLPC_BITMAP,0,XLPC,REC)				    !V02
C
C UPDATE THE FILE.
C
8000	CONTINUE

C
C SUBROUTINE X2NPCADD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2NPCADD.FOV                                 $
C  $Date::   17 Apr 1996 16:25:28                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2npcadd.for **
C
C X2NPCADD.FOR
C
...
C
C This program will add a Host Global address record.
C
C Calling sequence:
C
C     CALL X2NPCADD(REC)
...
C
C
	DO 100 I=2,X2XNPC_ENTRIES-1
110	  CONTINUE
	  BOT=X2XNPC_RANGE(1,I)
	  TOP=X2XNPC_RANGE(2,I)
	  IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	    ENDOFF=0
	    IF(X2XNPC_VALUE(1,I).NE.-1) THEN
	      DO 112 J=1,15
	        IF(X2XNPC_VALUE(J,I).NE.-1) THEN
	          ENDOFF=J
	        ENDIF
112	      CONTINUE
	      WRITE(5,9025) (X2XNPC_VALUE(J,I),J=1,ENDOFF)
	    ENDIF
C
C PROMPT FOR INPUT DATA.
C
	    WRITE (PROMPT2,9030) X2XNPC_FIELD(I),BOT,TOP
	    CALL INPNUM(PROMPT2,X2XNPC_REC(X2XNPC_INDEX(I)),
     *	                BOT,TOP,EXT)
	    IF(EXT.EQ.-9) THEN
...
C
	    IF(X2XNPC_VALUE(1,I).NE.-1) THEN
	      VALID=.FALSE.
	      DO 120 J=1,15
	        IF(X2XNPC_REC(X2XNPC_INDEX(I)).EQ.X2XNPC_VALUE(J,I))
     *	          VALID=.TRUE.
120	      CONTINUE
	      IF(.NOT.VALID) THEN
	        WRITE(5,9070) CHAR(7)
...
C
	  ELSE IF(BOT.EQ.-2) THEN
	    WRITE (PROMPT2,9032) X2XNPC_FIELD(I), TOP
	    CALL WIMG(5,PROMPT2)
	    READ(5,9060) CHRSTR
C
	    NUMCHR=0
...
122	    CONTINUE
          CALL ATOH(CHRSTR,1,NUMCHR,X2XNPC_REC(X2XNPC_INDEX(I)),ERR)
	    IF(ERR.NE.0) THEN
	      WRITE(5,9080) CHAR(7)
	      GOTO 110
	    ENDIF
...
C
	...
	X2XNPC_PORT=REC
CV02	X2XNPC_BITMAP=-1
        CALL X2BSET(X2XNPC_BITMAP,0,XNPC,REC)				  !V02
C
C UPDATE THE FILE.
C
8000	CONTINUE

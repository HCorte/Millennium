...
C --- ---------- ------- ----------------------------------------------
C V01 2014.02.27  SCML   Created - PLACARD Project - Creates suppression 
...
C=======OPTIONS /CHECK=NOOVERFLOW/EXT
        SUBROUTINE OIGSSUPR(WRKBUF, TRABUF, MESS, MES_LEN, BUFNUM, TER, ST)
        IMPLICIT NONE
...
C----+------------------------------------------------------------------
        ...
        IF(IGSDEBUG(IA_OUTIGS)) THEN
            CALL OPS('OIGSSUPR: ITYPE   ',ITYPE,ITYPE)
            CALL OPS('OIGSSUPR: ISUBTYPE',ISUBTYPE,ISUBTYPE)
            CALL OPS('OIGSSUPR: OTYPE   ',OTYPE,OTYPE)
            CALL OPS('OIGSSUPR: OSUBTYPE',OSUBTYPE,OSUBTYPE)
        ENDIF
     ...
     *                    , SUBTYP
     *                    , IGS_MILSYS
     *                    , 'MIL-00003' ! Function Suppressed
C     *                    , 'Função suprimida.')
     *                    , 'FunÃ§Ã£o suprimida.' ! UTF-8 Encoded
     *                    , 0 ) ! Don't use XREF from WRKTAB
            CALL OPSTXT('FUNCTION SUPPRESSED')
        ENDIF

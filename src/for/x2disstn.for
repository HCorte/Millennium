C
C SUBROUTINE X2DISSTN
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2DISSTN.FOV                                 $
C  $Date::   17 Apr 1996 16:15:34                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2disstn.for **
C
C X2DISSTN.FOR
C
...
C Calling Sequence:
C
C     CALL X2DISSTN(TRABUF,MESS,ORGMESS,LEN)
C
C Input parameters:
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2DISSTN(TRABUF,MESS,ORGMESS,MESLEN)
	IMPLICIT NONE
C
...
C
	...
	INTEGER*4   J, LOOPCNT, DATALEN, I, FEHDRLEN, TXHDRLEN
	INTEGER*4   CONN_TYPE,FLAGS,NET_TYPE,OPTIONS
	INTEGER*4   MESTYP          !Front End message data unit type
C
...
C
	...
	TXHDRLEN=TXHDRLEN-1
C
C GET THE FE MESSAGE DATA UNIT TYPE				!V02
C
	CALL ILBYTE(MESTYP,ORGMESS,X2FEMES_MESTYP-1+TXHDRLEN)	!V02
	IF (MESTYP .NE. X2FEMES_MESTYP_CMD) THEN  !PROCESS AS USUAL - V02
C
C GET THE FE HEADER LENGTH AND THE BAD ADDRESS LENGTH.
C
	    CALL ILBYTE(FEHDRLEN,ORGMESS,X2FEMES_HEADLEN-1+TXHDRLEN)
	    CALL ILBYTE(ADRLEN,ORGMESS,X2FEMES_ADRLEN-1+TXHDRLEN)
C
	    OUTBYT=0
	    TEMP=0
	    MESLEN=0
...
C
	    DO 100 I=TXHDRLEN+1,(TXHDRLEN+1)+FEHDRLEN-1
	      OUTBYT=OUTBYT+1
...
C
	    CALL ISBYTE(X2STMES_BAD_ADRESS,MESS,X2STMES_HDRLEN+FEHDRLEN+
     *	                                    X2STMES_CMD-1)
	    CALL ISBYTE(0,MESS,X2STMES_HDRLEN+FEHDRLEN+
     *	                                    X2STMES_BADADR_CNTNO-1)
            CALL I4TOBUF2(TRABUF(TXFVS),MESS,X2STMES_HDRLEN+FEHDRLEN+
     *                                      X2STMES_VS_SENT-1)
	    CALL ISBYTE(ADRLEN,MESS,X2STMES_HDRLEN+FEHDRLEN+
     *	                                    X2STMES_BADADR_ADRLEN-1)
	    LSTBYT=X2STMES_HDRLEN+FEHDRLEN+X2STMES_BADADR_ADDRESS
	    DATALEN=5
	    LOOPCNT=ADRLEN/2
...
C
	    MESLEN=LEN
	ELSE         !MESSAGE = X2FEMES_MESTYP_CMD SO CALL NEW ROUTINE - V02
            CALL X2DISCMD (TRABUF,MESS,ORGMESS,MESLEN,TXHDRLEN)	!V02

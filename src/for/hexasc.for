C
C SUBROUTINE HEXASC
C $Log:   GXAFXT:[GOLS]HEXASC.FOV  $
C  
C     Rev 1.0   17 Apr 1996 13:32:12   HXK
C  Release of Finland for X.25, Telephone Betting, Instant Pass Thru Phase 1
C  
C     Rev 1.0   21 Jan 1993 16:35:08   DAB
C  Initial Release
...
C Calling sequence:
C
C     CALL HEXASC(ASCSTR,START,LENGTH,INTSTR,ERR)
C
C Input parameters:
C
C     INTSTR      Int*4(2)    HEXADECIMAL number
C     START       Int*4       Starting HEX digit (2 digits per byte)
C     LENGTH      Int*4       Number of HEX digits to convert
C
C Output parameters:
C
C     ASCSTR      Char*1(*)   ASC string of converted HEX
C     ERR         Int*4       Return error
C                             -1 = invalid string
C                             -2 = invalid length
C
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE HEXASC(ASCSTR,START,LENGTH,INTSTR,ERR)
	IMPLICIT NONE
C
	INCLUDE 'INCLIB:SYSPARAM.DEF'
	INCLUDE 'INCLIB:SYSEXTRN.DEF'
C
	CHARACTER   ASCSTR(*)*1             !Ouptut ASCII string
	INTEGER*4   START                   !Start position into string
	INTEGER*4   LENGTH                  !Length to convert
	INTEGER*4   INTSTR(2)               !Input HEX value
...
C
     ...
     *	       ICHAR(VAL1).GT.57) THEN
	       VAL1=CHAR(ISHFT(VALUE,-4)+55)
	       IF(ICHAR(VAL1).LT.65.OR.
     *	          ICHAR(VAL1).GT.70) THEN
	         ERR=-1
	         GOTO 8000
	       ENDIF
	    ENDIF
...
C
     ...
     *	       ICHAR(VAL2).GT.57) THEN
	       VAL2=CHAR(IAND(VALUE,'0F'X)+55)
	       IF(ICHAR(VAL2).LT.65.OR.
     *	          ICHAR(VAL2).GT.70) THEN
	         ERR=-1
	         GOTO 8000
	       ENDIF
	    ENDIF

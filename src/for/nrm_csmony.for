...
C V01 15-FEB-1991 WOL INITIAL RELEASE
C
C THIS CHARACTER FUNCTION
C
C Calling sequence:
C
C     STRING=CSMONY(AMOUNT,LEN,FACTOR)
C
C Input parametrs:
C
...
C Output parameters:
C
C     CSMONY     Char*32       !THE STRING TO BE PRINTED ON REPORTS
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	CHARACTER FUNCTION CSMONY*32(AMOUNT,LEN,FACTOR)
	IMPLICIT NONE
    	INCLUDE 'INCLIB:SYSPARAM.DEF'
        INCLUDE 'INCLIB:GLOBAL.DEF'
C
C
	INTEGER*4	AMOUNT		!AMOUNT IN CENTS
...
C
	...
	ENDIF
        BIG=DFLOAT(AMOUNT)*MON_UNIT/DOLL_BASE

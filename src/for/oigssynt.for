...
C --- ---------- ------- ----------------------------------------------
C V01 2014.05.20  SCML   Created - PLACARD Project - Creates syntax error 
...
C=======OPTIONS /CHECK=NOOVERFLOW/EXT
        SUBROUTINE OIGSSYNT(WRKBUF, TRABUF, MESS, MES_LEN, BUFNUM, TER, ST)
        IMPLICIT NONE
...
C----+------------------------------------------------------------------
        ...
        IF(IGSDEBUG(IA_OUTIGS)) THEN
            CALL OPS('OIGSSYNT: ITYPE   ',ITYPE,ITYPE)
            CALL OPS('OIGSSYNT: ISUBTYPE',ISUBTYPE,ISUBTYPE)
            CALL OPS('OIGSSYNT: OTYPE   ',OTYPE,OTYPE)
            CALL OPS('OIGSSYNT: OSUBTYPE',OSUBTYPE,OSUBTYPE)
        ENDIF
     ...
     *                    , 'Erro de sintaxe.' ! UTF-8 Encoded
     *                    , 0 ) ! Don't use XREF from WRKTAB
            CALL OPSTXT('SYNTAX ERROR')
        ENDIF

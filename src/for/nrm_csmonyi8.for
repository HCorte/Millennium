...
C Calling sequence:
C
C     STRING=CSMONYI8(AMOUNT,LEN,FACTOR)
C
C Input parametrs:
C
...
C Output parameters:
C
C     CSMONYI8   Char*32       !THE STRING TO BE PRINTED ON REPORTS
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	CHARACTER FUNCTION CSMONYI8*32(AMOUNT,LEN,FACTOR)
	IMPLICIT NONE
...
C
        IF (FACTOR .EQ. VALUNIT .OR. FACTOR .EQ. VALUNIT) THEN
          MON_UNIT=DYN_VALUNIT
        ELSE IF (FACTOR .EQ. BETUNIT .OR. FACTOR .EQ. BETUNIT) THEN
...
C
	IF(CENT_LEN.EQ.0) THEN
	   DOLL_LEN = LEN
	ELSE
	   DOLL_LEN = LEN - CENT_LEN - 2
	ENDIF
        BIG=(DFLOAT(AMOUNT(1))*MON_UNIT+DFLOAT(AMOUNT(2)))/DOLL_BASE
        IF(DABS(BIG) .LT. 10.0**(DOLL_LEN+1)) THEN
	  IF(CENT_LEN.EQ.0) THEN

...
C GENPSORC.FOR
C
C V05 04-AGO-2015 SCML Portal prizes processing fix.
C V04 24-APR-2014 SCML Included validation of prize amount >= 150 in 
C                      PORTALSAP agents prizes. This prizes are now 
C                      settled as RECTYP = 04 instead of 03, and 
C                      APRTYP = 5.
...
C
      ...
      INTEGER*4 ST,SER,SERCNT,SERLOW,SERTOP
      INTEGER*4 LOGREC(LMUREC)
      LOGICAL*1 EOF
      CHARACTER RETNAM*19,VENNAM*19,PRMNAM*19
...
C
C Open PASORC Files
      CALL OPEN_PASORC(RET,RETLUN,RETNAM)
      CALL OPEN_PASORC(VEN,VENLUN,VENNAM)
      CALL OPEN_PASORC(PRM,PRMLUN,PRMNAM)
C
C Write PASORC Headers
      CALL WRITE_PASORC('HP',TRABUF,RET,RETLUN,RETNAM)
      CALL WRITE_PASORC('HP',TRABUF,VEN,VENLUN,VENNAM)
      CALL WRITE_PASORC('HP',TRABUF,PRM,PRMLUN,PRMNAM)
C
C Open VPF Files
...
1000  CONTINUE
      ...
      IF(EOF) GOTO 2000
C
      IF(MOD(SERCNT,SERTOP) .EQ. 0) THEN
        TYPE*,IAM(),'Reading MTM records from ',SERLOW,' to ',SERTOP
        SERLOW = SERTOP
        SERTOP = SERLOW + 1000000
      ENDIF
C
...
C
     ...
     *  CALL WRITE_PASORC('02',TRABUF,RET,RETLUN,RETNAM)  !pTicket Returns
C
     ...
     *    CALL WRITE_PASORC('05',TRABUF,VEN,VENLUN,VENNAM)  !eTicket Sales
C
      ENDIF
C
      IF(TRABUF(TTYP) .EQ. TCAN .AND. TRABUF(TWEPOP) .EQ. EPASSAL .AND.
     *   TRABUF(TSTAT) .EQ. GOOD)
     *  CALL WRITE_PASORC('06',TRABUF,VEN,VENLUN,VENNAM)  !eTicket Cancels
C
      IF(TRABUF(TTYP) .EQ. TVAL .AND. TRABUF(TSTAT) .EQ. GOOD .AND.
     *   TRABUF(TERR) .EQ. NOER) THEN
        IF(TRABUF(TVTYPE) .EQ. VPNBNK) THEN
          CALL WRITE_PASORC('04',TRABUF,PRM,PRMLUN,PRMNAM)  !xTicket Bank Transfers
        ELSE
          CALL WRITE_PASORC('03',TRABUF,PRM,PRMLUN,PRMNAM)  !xTicket Payments
        ENDIF
      ENDIF
C
      GOTO 1000
...
C Write PASORC Trailers
      CALL WRITE_PASORC('TP',TRABUF,RET,RETLUN,RETNAM)
      CALL WRITE_PASORC('TP',TRABUF,VEN,VENLUN,VENNAM)
      CALL WRITE_PASORC('TP',TRABUF,PRM,PRMLUN,PRMNAM)
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
      SUBROUTINE OPEN_PASORC(ORCIND,ORCLUN,ORCNAM)
      IMPLICIT NONE
C
      INCLUDE 'INCLIB:SYSPARAM.DEF'
      INCLUDE 'INCLIB:SYSEXTRN.DEF'
...
C
C=======OPTIONS /CHECK=NOOVERFLOW
      SUBROUTINE WRITE_PASORC(RECTYP,TRABUF,ORCIND,ORCLUN,ORCNAM)
      IMPLICIT NONE
C
      INCLUDE 'INCLIB:SYSPARAM.DEF'
      INCLUDE 'INCLIB:SYSEXTRN.DEF'
...
C Trailer
      ELSEIF(RECTYP .EQ. 'TP') THEN
        ...
        TYPE*,IAM(),ORCNAM,': ',RECCNT(ORCIND),' records written'
C
C pTicket Returns
...
C
      ...
      INTEGER*4 TCKS,TCKN,NUMFRACS,HALF,INIFRACS,CNTFRACS
...
C
          ...
          ELSEIF(TRABUF(TPRETYP) .EQ. BYFRAC) THEN
            INIFRACS = TRABUF(TPTEN1 + OFFTRA*(TCKS-1))
            NUMFRACS = INIFRACS
...
C
      ...
      INTEGER*4 ST,TCKS,EMIS,EIND,ECNT,WEEK,YEAR
      INTEGER*4 EXTSER,CHKDIG
      INTEGER*4 PZIND,PZDIV(6)
      INTEGER*2 DATE(DATLEN)
      CHARACTER RECTYP*2
      CHARACTER PRECTYP*2 !V05
      CHARACTER HHMMSS*6,GET_HHMMSS_TIME*6

C----|--!--------------------------------------------------------------
C V04   ! Portalsap validation prize amount - start
C----|--!--------------------------------------------------------------
      INTEGER*4  PORTALSAP
...
C V04   ! Portalsap validation prize amount - start
C----|--!--------------------------------------------------------------
C         Exception for portal prizes >= 150 euros and < 5000 euros
          IF(SAPN .EQ. PORTALSAP) THEN          !Portal agents
             IF(TRABUF(TPPAY1 + OFFTRA*(TCKS-1)) .GE. P(VALORDER) 
     *          .AND. TRABUF(TPPAY1 + OFFTRA*(TCKS-1)) .LT. P(VALPRZHI)) THEN
C                RECTYP = '04'                   !Force to be Bank Transfer type
                PRECTYP = '04'                   !Force to be Bank Transfer type !V05
                IF(TSBIT(AGTTAB(AGTTYP,TERN),AGTPRV)) THEN    !Not Expected
                   APRTYP = 6                   !Payments by Bank Transfer directly to the Player from privileged terminals
                ELSE
                   APRTYP = 5                   !Force to be Payments by Bank Transfer directly to the Player
                ENDIF
             ELSE
C                RECTYP = '03'                   !xTicket Payments
                PRECTYP = '03'                   !xTicket Payments
                IF(TSBIT(AGTTAB(AGTTYP,TERN),AGTPRV)) THEN
                   APRTYP = 1                   !Payments to players from privileged terminals
                ELSE
                   APRTYP = 3                   !Payments to players from agents (non-privileged terminals)
                ENDIF
             ENDIF
          ENDIF
C----|--!--------------------------------------------------------------
C V04   ! Portalsap validation prize amount - end
C----|--!--------------------------------------------------------------

          RECCNT(ORCIND) = RECCNT(ORCIND) + 1
C
          IF(SAPN .EQ. PORTALSAP) THEN          !Portal agents
            WRITE(ORCLUN,100) PRECTYP,
     *                        PSVTYP,
     *                        WEEK,
     *                        YEAR,
     *                        TCKTYP,

...
C
C  $Logfile::   GXAFXT:[GOLS]X2PRINT_REPY.FOV                             $
C  $Date::   17 Apr 1996 16:26:50                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
9200	FORMAT ('X2TERMSTAY',I2.2,'.REP')
C	FILENAME='X2TERMSTAY'//'03'//'.REP'
	OPEN(6,FILE=FILENAME,RECL=132,
     *	       STATUS='NEW',     IOSTAT=ST)
	IF(ST.NE.0) THEN
	  CALL OS32ER(5,'X2TERMSTAY.REP','OPEN',ST,0)
	  CALL GPAUSE
	ENDIF
C
...
C
     ...
     *			  /1000
	      ...
	      IF (STATION_STATE.LE.X2XS_IDLE) GOTO 1000 !!!!!!.AND. 

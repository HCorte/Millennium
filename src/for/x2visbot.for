C
C SUBROUTINE X2VISBOT
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2VISBOT.FOV                                 $
C  $Date::   17 Apr 1996 16:39:58                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2VISBOT(SCRIDX,LEVEL,DUMMY,LEN,ERR)
	IMPLICIT NONE
C
...
C
	ERR=0
	X2FLDINF(XBOTIDX)=1
	X2SCRN(X2SCRN_KEYLEV3+LEVEL,SCRIDX)=XBOTIDX
C
	RETURN
	END

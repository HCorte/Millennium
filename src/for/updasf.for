...
C UPDASF.FOR                                                                    
C
C V13 18-DEC-2015 FRP CR31 PIN for Owner and Clerk
C V12 14-AGO-2014 BMV Set enable status in XML file when x25add is dif then zero
C V11 27-MAR-2014 FRP Set disable status in XML file for offline terminals
C V10 20-JUN-2013 FJG New Terminal Bid
...
C=======OPTIONS/CHECK=NOOVERFLOW/EXT
      ...
      INTEGER*4    ENABLED, DISABLED  !V11
      PARAMETER   (ENABLED  = 1)
      PARAMETER   (DISABLED = 2)
      
      INTEGER*4    ASF2ESC(NUMAGT)
      INTEGER*4    STA2ESC(NUMAGT)    !V11
      INTEGER*2    CNT2ESC
...
C-----V10-----------------------------------------------------------------------
      CNT2ESC = 0
      CALL FASTSET(0,ASF2ESC,NUMAGT)
      CALL FASTSET(0,STA2ESC,NUMAGT)  !V11
C-----V10-----------------------------------------------------------------------
      CALL COPYRITE                                                             
  	                                                                        
      TYPE *                                                                    
      TYPE *,'----------------------------------------------------------------------------'
      TYPE *,'<<<<< Atualiza��o de dados de Agente a partir da Base de Dados da SCML >>>>>'                  
      TYPE *,'----------------------------------------------------------------------------'
      TYPE *                                                                    
C
C     OPEN REPORT FILE
...
C
               ...
               SLOT = SLOT - 1
C--------------V10--------------------------------------------------------------               
C              ASF2ESC(TERMINAL) = AGTMIL_REC.AGENT
C              CNT2ESC = CNT2ESC + 1
C--------------V10--------------------------------------------------------------               
            ENDIF
...
C-----------V11-----------------------------------------------------------------
            IF(AGTMIL_REC.AGENT_TYPE.EQ.'2') THEN   !OFFLINE TERMINAL
              ASF2ESC(TERMINAL) = AGTMIL_REC.AGENT
              STA2ESC(TERMINAL) = DISABLED
              CNT2ESC = CNT2ESC + 1
            ELSE
              IF(AGTMIL_REC.OPER_TYPE.EQ.'1') THEN  !NEW TERMINAL
                ASF2ESC(TERMINAL) = AGTMIL_REC.AGENT
                STA2ESC(TERMINAL) = ENABLED
                CNT2ESC = CNT2ESC + 1
              ENDIF
            ENDIF
C-----------V11-----------------------------------------------------------------
...
C           CALL BINASC(ASFINF, SPAS8, LPAS8, AGTMIL_REC.AGENT_PASSWORD)
            CALL BINASC(ASFINF, SPAS8, LPAS8, AGTMIL_REC.MANAGER_PASSWORD)  !USE PAS8 FOR OWNER PASSWORD (CR31)
C
...
C--------------V10--------------------------------------------------------------               
            IF (AGTMIL_REC.OPER_TYPE.EQ.'1') CALL BSET(TERMTYP,AGTMXT) !meter a 1 (set) o 9º bit
C--------------V10--------------------------------------------------------------                           
...
C
C                 For Passive Validations (and RETURNS) are always enabled
C
...
C
C--------------V12--------------------------------------------------------------
            IF (AGTMIL_REC.X2XADDRESS .NE.'000000000' .AND. CASFBYT(SXADR:EXADR) .EQ. '000000000') THEN 
                ASF2ESC(TERMINAL) = AGTMIL_REC.AGENT
                STA2ESC(TERMINAL) = ENABLED
                CNT2ESC = CNT2ESC + 1
            ENDIF
C--------------V12--------------------------------------------------------------
	    ...
	    CASFBYT(SSCLS:ESCLS) = '02'    !STN CLS
...
C-----------V11-----------------------------------------------------------------
            ASF2ESC(TERMINAL) = AGTMIL_REC.AGENT  !DELETED TERMINAL
            STA2ESC(TERMINAL) = DISABLED
            CNT2ESC = CNT2ESC + 1
C-----------V11-----------------------------------------------------------------
...
500   CONTINUE     !END OF AGTMIL FILE LOOP *** 
     ...
     *                          'Goto to UPDASF.REP report for it', 0, ' ', 0) 
      ENDIF
C--------------V10--------------------------------------------------------------               
      IF(CNT2ESC.GT.0) CALL ASF2ESC_SUB(ASF2ESC,STA2ESC)
C--------------V10--------------------------------------------------------------                                                                                              
      CALL GSTOP(GEXIT_SUCCESS)                          
...
500     CONTINUE     !END OF AGTMIL FILE LOOP *** 
	...
	END
C--------------V10--------------------------------------------------------------
	SUBROUTINE ASF2ESC_SUB(ASF2ESC,STA2ESC)               
        IMPLICIT NONE   
...
C-------OPEN ESC_PT_CDC.XML
	...
	ELSE
	  CNT=0
          ...
          DO AGT=1,NUMAGT
            IF (ASF2ESC(AGT).GT.0) THEN
              IF(STA2ESC(AGT).EQ.ENABLED) THEN
                WRITE(1,1200) AGT,ASF2ESC(AGT)              
              ELSEIF(STA2ESC(AGT).EQ.DISABLED) THEN
                WRITE(1,1201) AGT,ASF2ESC(AGT)              
              ELSE
                GOTO 9000
              ENDIF
              CNT=CNT+1

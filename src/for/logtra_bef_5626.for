C
C SUBROUTINE LOGTRA_BEF_5626
C
C LOGTRA_BEF_5626.FOR (SINCE V58)
C
C V58 04-JUL-2016 SCML M16 PROJECT
C-----------------------------------------------------------------------
...
C INTERNAL FORMAT BEFORE M16 PROJECT (SINCE V58)
C
C
C CALLING SEQUENCE:
C     CALL LOGTRA_BEF_5626(TRABUF,LOGBUF)
C INPUT
...
C=======OPTIONS /CHECK=NOOVERFLOW
        SUBROUTINE LOGTRA_BEF_5626(TRABUF,LOGBUF)
        IMPLICIT NONE
C
...
C
              LOGOFF = 17
              BRDOFF = 15                       !HOLE RECORD FOR BOARD DATA
C                                               !IF WE DO NOT PLAY KICKER.
              IF(KIKFLG) THEN
                 TRABUF(TWKICK)  = LOGBUF(17)
                 TRABUF(TWKICK2) = LOGBUF(18)
C
                 I4TEMP = LOGBUF(19)
...
C
            CALL MOVBYT(LOGBUF(10),1,BUFF(1),1,24) !gams 1 to 8 (3 bytes each)
C
            IF(TRABUF(TIBCH).GE.9)
     *        CALL MOVBYT(LOGBUF(17),1,BUFF(25),1,60) !gams 9 to 28
            IF(TRABUF(TIBCH).GE.29)
     *        CALL MOVBYT(LOGBUF(33),1,BUFF(85),1,36) !gams 29 to 40
C
            IND=1

C
C SUBROUTINE X2LPCLIS
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2LPCLIS.FOV                                 $
C  $Date::   21 May 1996 20:00:28                                         $
C  $Revision::   1.1                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C  Based on Netherlands Bible, 12/92, and Comm 1/93 update
C  DEC Baseline
C
C ** Source - x2lpclis.for;1 **
C
C X2LPCLIS.FOR
C
C V01 01-DEC-91 DAS RELEASED FOR VAX (NETHERLANDS)
C
C This subroutine will display defined ports in the
C local port configuration file to the screen.  The user
C will have the option of displaying a specific port,
C or to page forward or back.
C
C Calling sequence:
C
C     CALL X2LPCLIS
C
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C This item is the property of GTECH Corporation, Providence, Rhode
C Island, and contains confidential and trade secret information. It
C may not be transferred from the custody or control of GTECH except
C as authorized in writing by an officer of GTECH. Neither this item
C nor the information it contains may be used, transferred,
C reproduced, published, or disclosed, in whole or in part, and
C directly or indirectly, except as expressly authorized by an
C officer of GTECH, pursuant to written agreement.
C
C Copyright 1994 GTECH Corporation. All rights reserved.
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2LPCLIS
	IMPLICIT NONE
C
	INCLUDE 'INCLIB:SYSPARAM.DEF'
	INCLUDE 'INCLIB:SYSEXTRN.DEF'
C
	INCLUDE 'INCLIB:X2XPRM.DEF'
	INCLUDE 'INCLIB:GLOBAL.DEF'
	INCLUDE 'INCLIB:X2XLPC.DEF'
C
	INTEGER*4   LPC,LPCCNT          !Rec #/station count
	INTEGER*4   LASTLPC             !Last station displayed
	INTEGER*4   EOFCNT              !End of file count
	INTEGER*4   ST,K                !Work variables
	INTEGER*4   LAST_PREV, PREV
	CHARACTER   PROMPT*65           !Output prompt
	CHARACTER   OPT*4               !Input option
	CHARACTER   X2FILNAM*20         !File name function
	CHARACTER   STATE(0:3)*10       !Port state
	LOGICAL     MORE                !More data flag
C
	DATA        STATE /' undefined','      idle',
     *	                   '    online','      down'/
C
C DISPLAY THE NEXT SCREEN OF PORTS.
C
	LPC=0
	LASTLPC=0
	PREV=0
	LAST_PREV=0
100	CONTINUE
	IF(LPC.LT.0) LPC=0
	EOFCNT=0
	LPCCNT=0
	CALL CLRSCR(5)
	WRITE(5,9080)
	WRITE(5,9090)
C
C READ A STATION FROM THE FILE AND DISPLAY
C IT TO THE SCREEN.
C
200	CONTINUE
	IF(LPCCNT.LT.10 .AND. EOFCNT.LE.100) THEN
250	  LPC=LPC+1
	  CALL READW(X2XLPC_FDB,LPC,X2XLPC_REC,ST)
	  IF(ST.EQ.144) GOTO 300
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XLPC),'READW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  IF(X2XLPC_REC(1).LE.0) THEN
	    GOTO 250
C         EOFCNT=EOFCNT+1
C         IF(EOFCNT.LE.100) GOTO 250
C         GOTO 300
	  ELSE
	    EOFCNT=0
	  ENDIF
C
C DISPLAY THE PORT TO THE SCREEN.
C
	  LPCCNT=LPCCNT+1
	  IF(LPCCNT.EQ.1) THEN
	    LAST_PREV=PREV
	    PREV=LPC
	  ENDIF
	  LASTLPC=LPC
	  WRITE(5,9100) X2XLPC_PORT,     X2XLPC_SAP,
     *	                X2XLPC_SAP_PORT, STATE(X2XLPC_STATE),
     *	                X2XLPC_NETPORT
	  GOTO 200
	ENDIF
C
C IF NO PORTS EXIST, DISPLAY AN APPROPRIATE MESSAGE.
C
300	CONTINUE
	IF(LPCCNT.EQ.0) THEN
	  WRITE(5,9050)
	ENDIF
C
C BUILD THE APPROPRIATE OUTPUT PROMPT.
C
	IF(EOFCNT.LE.100) THEN
	  WRITE (PROMPT,9000)
	  MORE=.TRUE.
	ELSE
	  WRITE (PROMPT,9010)
	  MORE=.FALSE.
	ENDIF
C
C ASK USER WHAT THEY WANT TO DO.
C
	WRITE(5,*)
	WRITE(5,*)
350	CONTINUE
	CALL WIMG(5,PROMPT)
	READ(5,9020) OPT
	IF(OPT(1:1).EQ.'E'.OR.OPT(1:1).EQ.'e') THEN
	  GOTO 8000
	ELSE IF(MORE.AND.(OPT(1:1).EQ.'N'.OR.OPT(1:1).EQ.'n')) THEN
	  LPC=LASTLPC
	  GOTO 100
	ELSE IF(OPT(1:1).EQ.'P'.OR.OPT(1:1).EQ.'p') THEN
	  LPC=LAST_PREV-1
	  GOTO 100
	ELSE IF(OPT(1:1).EQ.'X'.OR.OPT(1:1).EQ.'x') THEN
	  LPC=CTOI(OPT(2:4),K)-1
	  GOTO 100
	ELSE IF(OPT(1:1).EQ.'H'.OR.OPT(1:1).EQ.'h') THEN
	  CALL X2XHLP('X2LPCLIS.HLP')
	  LPC=PREV-1
	  GOTO 100
	ELSE
	  WRITE(5,9040)
	  GOTO 350
	ENDIF
C
C PROGRAM EXIT
C
8000	CONTINUE
	RETURN
C
C     ==================== Format Statements ===================
C
9000	FORMAT(15(' '),'Enter N for next, P for Previous, ',
     *	               'or X# for port')
9010	FORMAT(15(' '),'Enter P for previous or X# for port',13(' '))
9020	FORMAT(A4)
9040	FORMAT(15(' '),'Invalid option input ')
9050	FORMAT(15(' '),'Sorry, no more ports exist ')
9080	FORMAT(//,T26,'GTECH Distributed Network',/,
     *	          T29,'Local Port Listing',//)
9090	FORMAT(T9,'Port',T19,'SAP',T28,'SAP Port',
     *	       T40,'Port State',T54,'Network Port',/,8X,57('='))
9100	FORMAT(T8,I5,T19,I3,T30,I3,T40,A10,T55,I5)
	END

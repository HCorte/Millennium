...
C CWINSEL.FOR
C
C TODAY'S COUPLE WINNER SELECTION PROGRAM.
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C---- Initialize winner selection common.
     ...
     *            'This program can be run only from *WINTSK or MULTIWIN'
        ...
        IF(STOPMOD.EQ.WINMANUAL) THEN
             CALL CWIN_WININT(CFILES,FILCNT)
        ELSE
             NTSK=INDTSK('CWINTSK ')
	...
	IF(FILCNT.EQ.0) THEN
	  TYPE*,IAM(),' Sorry, no Todays Couple winner selection today'
C	  CALL GSTOP(GEXIT_FATAL)
	ENDIF

	CALL CWIN_WINLOD(1,DUMMY)
...
C---- Start draw file scan.
	...
	DO 3000 I = 1,FILCNT
	  CALL CWIN_OPNDRW(FILES(1,I),PTMF)
...
2030	  CONTINUE
	  ...
	  IF (GTYP .NE. TCPL) GOTO 2030

C---- Check if winner.

	  CALL CWIN_POST(TRABUF)

	  CALL CCHKWIN(TRABUF,V4BUF,WIN)

	  IF (WIN .NE. 0) THEN
	    CALL CWIN_WINLOD(2,V4BUF)
	    CALL FASTSET(0,V4BUF,VFLEN*4)
	  END IF

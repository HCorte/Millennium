...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE GUICMD_007(OUTBUF,MES_LEN,RET_CODE)
	IMPLICIT NONE
C
...
C
	...
	INTEGER*4 STATUS,LEN,I,TEMP
	CHARACTER*40 STATUS_STR

	INTEGER*4 CNT, TER, AGT, IDX
	BYTE      AMSG(512)
	INTEGER*4 BEG
	PARAMETER(BEG=0*29)
C
...
C Get agent number
C
	IF(GUI_ARGLEN(2).GT.0) THEN
	   AGT = CTOI(GUI_ARGCHAR(2),ST)
        ELSE
           AGT = AGTTAB(AGTNUM,TER)
	ENDIF
	CALL FIND_AGENT(AGT,TEMP,ST)
	IF(ST.EQ.0) THEN
           IF(TEMP.NE.TER) THEN
	      STATUS_STR = 'Duplicate agent number'
	      STATUS = 2
	      GOTO 100
	   ENDIF
	ELSE       
           IF(AGTTAB(AGTNUM,TER).NE.0) THEN   ! CHANGING AGENT NUMBER
              STATUS_STR = 'Cannot change an agent number'
	      STATUS = 3
	      GOTO 100
	   ELSE                               ! NEW AGENT
              STATUS_STR = 'Cannot add a new agent'
	      STATUS = 4
	      GOTO 100 
	   ENDIF
	ENDIF            

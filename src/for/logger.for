...
C
        MESS(1)=LOG
        CALL OPS('Channel Flag before TMFMON',TRABUF(TVOLMCOMF_IL),TRABUF(TVOLMCOMF_IL))        
C
10	CONTINUE
...
CEV10
		...
		ENDIF
	     ...
	     GOTO 9000
	  ENDIF
	  CALL OPS('Channel Flag before logout',TRABUF(TVOLMCOMF_IL),TRABUF(TVOLMCOMF_IL))
C
C UPDATE BUFFER STATUS AND RELEASE
C ALL TASKS WAITING FOR THIS BUFFER.
C

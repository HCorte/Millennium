C  GXSRC:DELLVL.FOR
C  
C  $Log:   GXAFXT:[GOLS]DELLVL.FOV  $
C  
C     Rev 1.1   19 May 1996 17:53:22   HXK
C  Applying PVCS header for automatic revision history
C  
C     Rev 1.0    19 May 1996 17:49:02   HXK
C  Initial revision.
C  
C
C SUBROUTINE DELLVL
...
C
	...
	CHARACTER   CNUM*5, CNUM1*5, CNUM2*5, LEV1*5, COMMA
...
C
C Make sure number is in the file                            
C
	DO 70 I=1,LVLIM
          IF(LEV(LEVEL,PROG,I).EQ.CNUM) THEN
	    DO 75 J=I,(LVLIM-1)
	      LEV(LEVEL,PROG,J)=LEV(LEVEL,PROG,J+1)
75	    CONTINUE
	    LEV(LEVEL,PROG,J)=CNUM1
	    GOTO 80
...
940   FORMAT(///,28X,A25,//,5X,'Level ',I2.2,' contains the following',
     *       ' information......',/)
950     FORMAT(12(1X,A5))
960     FORMAT(//1X,' Number  ',A5,' not in the file ! ')
970     FORMAT(//1X,' Number  ',A5,' deleted from level ',I2.2)
        END

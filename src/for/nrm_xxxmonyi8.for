...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE XXXMONYI8(PRMFLG, STRING,NUM,FACTOR,EXT)
	IMPLICIT NONE
C
...
C
	INTEGER*4  INBUF(5), EXT, ST, FLG, NUM(2), DCNT, I
	INTEGER*4  DIG,  FACTOR, MON_UNIT, SIGN
	REAL*8     ONUM
	LOGICAL DPNT
	REAL*8  MAXI4
        DATA MAXI4/21474836479.0D0/
C
	INTEGER*4   INLEN
...
C
	IF(ST.LT.0.OR.FLG.EQ.0.OR.DCNT.NE.CENT_LEN) THEN
	  ...
	  GOTO 10
	ENDIF
	IF(ONUM.GT.MAXI4) THEN
	  WRITE(OUTLU,902) IAM()
	  GOTO 10
	ENDIF
C
	NUM(1) = ONUM / DFLOAT(MON_UNIT)
	ONUM = ONUM - DFLOAT(NUM(1)) * DFLOAT(MON_UNIT)
	NUM(2) = MOD(IDINT(ONUM),MON_UNIT)
	NUM(1) = NUM(1) * SIGN
	NUM(2) = NUM(2) * SIGN
C
C

...
C
C  $Logfile::   GXAFXT:[GOLS]X2TTNMEN.FOV                                 $
C  $Date::   17 Apr 1996 16:39:14                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - x2ttnmen.for;1 **
C
C X2TTNMEN.FOR
C
C V01 01-AUG-90 XXX RELEASED FOR VAX
C
C This program provides an interface into the TITAN class
...
C
	PROMPT=NULL
	WRITE (PROMPT,9020)
	CALL INPNUM(PROMPT,REC,X2XTTN_RANGE(1,1),
     *	            X2XTTN_RANGE(2,1),EXT)
...
C
	...
	IF(OPT.EQ.1) THEN
104	  CONTINUE
	  CALL READL(X2XTTN_FDB,REC,X2XTTN_REC,ST,LOCK)
	  IF(LOCK.NE.0) THEN
	    LOCKCNT=LOCKCNT+1
	    CALL XWAIT(1,2,ST)
	  ...
	  ENDIF
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XTTN),'READL',ST,REC)
	    CALL GPAUSE
	  ENDIF
	  IF(X2XTTN_CLASS.GT.0) THEN
	    WRITE(5,9032) REC,CHAR(7)
	    CALL X2UNLOCK(XTTN,X2XTTN_FDB,REC,X2XTTN_REC)
	    CALL XWAIT(2,2,ST)
	    GOTO 100
	  ENDIF
	  CALL X2TTNADD(REC)
...
9000	FORMAT(////,T26,'GTECH Distributed Network',/,
     *	          T30,'TITAN Class Menu',//,
     *	          T20,'  1. Add a TITAN class',/,
     *	          T20,'  2. Modify a TITAN class',/,
     *	          T20,'  3. Delete a TITAN class',/,
     *	          T20,'  E. Exit',//)
9010	FORMAT(25(' '),'Enter option [1-3] ')
9020	FORMAT(25(' '),'Enter class number   ')

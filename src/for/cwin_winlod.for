C
C SUBROUTINE CWIN_WINLOD
C $Log:   GXAFXT:[GOLS]CWIN_WINLOD.FOV  $
C  
C V01 14-JAN-99 GPW STOPSYS OPTIMIZATION
C
C     Rev 1.0   17 Apr 1996 12:46:52   HXK
C  Release of Finland for X.25, Telephone Betting, Instant Pass Thru Phase 1
C  
C     Rev 1.0   23 Nov 1995 14:18:08   PXB
C  Initial revision.
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	...
	SUBROUTINE CWIN_WINLOD(FUN,RECORD)
...
C---- Local variables.
	...
	INTEGER*4 FUN, VIND, VBLOCK, TIND, TBLOCK, VLFCNT, TCFCNT
	INTEGER*4 ST, K
...
100	CONTINUE
        ...
        CALL WINGETW('CWINTSK ',NAMVLW,NAMTCW)                    ! V01
	CALL OPENW(VLW,NAMVLW,4,0,0,ST)

	CALL IOINIT(VFDB,VLW,128*256)

	IF (ST .NE. 0) CALL FILERR(SFNAMES(1,VLW),1,ST,0)
...
C***	  IF(ST.NE.0) CALL FILERR(SFNAMES(1,TCW),3,ST,TBLOCK)

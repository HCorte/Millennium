...
C
C
C PETTY CASH TERMINAL REPORT SUBROUTINE
C
C CALLING SEQUENCE:
C     CALL SALPET(SALES,SUBCLASS,MANAGER,SALOFF,CDC,RTER,TRABUF)
C INPUT
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE SALPET(SALES,SUBCLASS,MANAGER,SALOFF,CDC,RTER,TRABUF)
	IMPLICIT NONE
...
C
	...
	TMPAMT(1) = 0
	TMPAMT(2) = 0
        OFF=PRM_NUMINS*2+PRM_NUMISAL !OFFSET INTO AGTMIS TABLE FOR PETTY ITEMS
	

C
C PROCESS ONLY PETTY CASH REPORT
C
	IF(SUBCLASS.EQ.0.OR.SUBCLASS.EQ.8) THEN  !TODAY OR W. T. D.
            SALIND=1
	    DO 200 I=OFF+1,OFF+PRM_NUMIPTY
...
C
	IF(MANAGER)  THEN   !GET CLERKS ACCOUNTS ALSO
	    DO 220 CLERK=2,8
	        ...
	        SALIND=1
                DO 210 I=OFF+1,OFF+PRM_NUMIPTY
	...
	IF(SUBCLASS.EQ.8) THEN
	    DO 240 I= 1,9
	        IF(ASFDAT(ASFCDC,I).LE.ASFINV(ASFEND,1)) GOTO 240
                SALIND=1
	        DO 230 J=OFF+1,OFF+PRM_NUMIPTY
	            SALES(SALIND)=SALES(SALIND)+
     *                            ASFMIS(J,1,I)
...
260	    CONTINUE
            SALIND=1
	    DO 270 J=OFF+1,OFF+PRM_NUMIPTY
	        SALES(SALIND)=SALES(SALIND)+
     *                        ASFMIS(J,1,I)
	        SALES(SALIND+1)=SALES(SALIND+1)+
     *                          ASFMIS(J,2,I)

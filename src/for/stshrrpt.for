...
C
        SUBROUTINE STSHRRPT(GNUM,GIND,DRAW,COPY)
C
        IMPLICIT NONE
C
...
C
        ...
        END IF
C
        CALL TITLE(HEAD,'STWINRP ',GIND,6,PAGE,CDC)
C
	WRITE(6,802) (SCFLGN(I,GNUM),I=1,4),DRAW,WEKNO,YEAR2,
     *               (DATE(K),K=7,13)
C
	WRITE(6,811) DSTWPA(1,PRWON)
C
        WRITE(6,921) (DSTENM(I),I=1,STRENM_LEN/4)
C
	IF ((DSTSTS.EQ.GAMCAN).OR.(DSTSTS.EQ.GAMREF)) THEN
	  WRITE (6,9301)
	  GOTO 9998
	END IF
C
...
C
	DO I=1,DSTCMB
          WRITE(6,9221) DSTWIN(1,I),(DSTNMS(K,DSTWIN(1,I)),K=1,4)
          WRITE(6,9222) DSTWIN(2,I),(DSTNMS(K,DSTWIN(2,I)),K=1,4)
          WRITE(6,9223) DSTWIN(3,I),(DSTNMS(K,DSTWIN(3,I)),K=1,4)
          WRITE(6,9224) DSTODS(I)/100,MOD(DSTODS(I),100)
	ENDDO
C
C CALCULATE PERCENTAGES AND DISPLAY TOTALS
C
	IF (DSTSAL(DOLAMT) .GT. 0) THEN
           IF(DSTSAL(DOLAMT) .EQ. DSTREF) THEN
              WINPER = 0.0
              WONPER = 0.0
           ELSE
 	      WINPER = (FLOAT(DSTWON-DSTREF)
     *                      /(DSTSAL(DOLAMT)-DSTREF))*100
 	      WONPER = (FLOAT((DSTWON-DSTREF)-DSTPOL(1))
     *                       /(DSTSAL(DOLAMT)-DSTREF))*100
           ENDIF
           IF(DSTWON.EQ.0) THEN
              ROLPER = (FLOAT(DSTTPL-DSTREF)/(DSTSAL(DOLAMT)-DSTREF))*100
           ELSE
              ROLPER = 0.0
           ENDIF
	ELSE
...
9998	CONTINUE
	...
	WRITE(6,901) CMONY(DSTSAL(DOLAMT),12,BETUNIT),
     *               CMONY(DSTREF,12,BETUNIT),
     *               CMONY((DSTSAL(DOLAMT)-DSTREF),12,BETUNIT),
     *               CMONY((DSTWON-DSTREF)-DSTPOL(1),12,BETUNIT),WONPER,
     *               CMONY(DSTPOL(1),12,BETUNIT),
     *               DSTWPR(1,PRWON),CMONY(DSTWON-DSTREF,12,BETUNIT),
     *               CMONY(DSTPOL(2),12,BETUNIT),ROLPER
C
9999	CONTINUE
C
        CALL USRCLOS1(6)
...
902	FORMAT('   ',I2,21(' '))
903	FORMAT(132A)
921     FORMAT(19X,'Kohde:      ',<STRENM_LEN/4>A4,/)
9221    FORMAT(19X,'Voittaja:   ',I2,'. ',4A4)
9222    FORMAT(19X,'Toinen:     ',I2,'. ',4A4)
9223    FORMAT(19X,'Kolmas:     ',I2,'. ',4A4,/)
9224    FORMAT(19X,'Kerroin:    ',I8,'.',I2.2,/)

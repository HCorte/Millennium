C
C SUBROUTINE FRMFCONF
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]FRMFCONF.FOV                                 $
C  $Date::   17 Apr 1996 13:13:42                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE FRMFCONF(SSAP,DSAP,BUF)
	IMPLICIT NONE
C
...
C
	IF(LOCLAN(SSAP,LAN).EQ.LANUP) THEN
	   ...
	   CALL I4TOBUF2(FRMLEN(FRFCONF),LANBUF(1,BUF),TYPEBEG-1)
	   CALL ISBYTE(DSAP,LANBUF(1,BUF),DSAPBEG-1)
	   CALL ISBYTE(SSAP,LANBUF(1,BUF),SSAPBEG-1)
	   CALL ISBYTE(CTRLUI,LANBUF(1,BUF),CTRLBEG-1)
	   CALL ISBYTE(FRFCONF,LANBUF(1,BUF),FRTYPBEG-1)
	   CALL ISBYTE(LAN,LANBUF(1,BUF),TLANBEG-1)
	   CALL ISBYTE(LANSAPSTS(SSAP),LANBUF(1,BUF),LANDATAB-1)
	   CALL MOVBYT(FLANHOME(1,LAN),1,LANBUF(1,BUF),LANDATAB+1,6)
	   CALL GOWRITE(BUF)
	   BUF=0
	ENDIF
C

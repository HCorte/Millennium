C
C SUBROUTINE X2RCDMOD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2RCDMOD.FOV                                 $
C  $Date::   17 Apr 1996 16:28:12                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C This program will modify a Statics Report code record.
C
C Calling sequence:
C
C     CALL X2RCDMOD(REC)
...
C
	...
	PRTFLG=.FALSE.
	CALL CLRSCR(5)
	CALL FASTMOV(X2XRCD_REC,DMYBUF,X2XRCD_SECT*64)
	WRITE(5,9000)
	WRITE (PROMPT,9020) X2XRCD_ENTRIES-1
C
...
C
50	CONTINUE
	LINES=X2XRCD_ENTRIES/2
	DO 100 I=1,LINES
	  OFFSET=(I-1)*2+1
C
C NOTE: OFFSET 3 IS HARDCODED TO ALLOW FIELD 4 (NAME)
C       TO BE CORRECTLY DISPLAYED.
C
	  IF(OFFSET.EQ.3) THEN
	    WRITE(5,9040) 3,X2XRCD_FIELD(3),X2XRCD_REC(3),
     *	                  4,X2XRCD_FIELD(4),
     *	                   (X2XRCD_DESC(J),J=1,X2XRCD_RANGE(2,4))
	    IF(PRTFLG)
     *	      WRITE(6,9040) 3,X2XRCD_FIELD(3),X2XRCD_REC(3),
     *	                    4,X2XRCD_FIELD(4),
     *	                     (X2XRCD_DESC(J),J=1,X2XRCD_RANGE(2,4))
	  ELSE
	    J=OFFSET
	    WRITE(5,9010) J,X2XRCD_FIELD(J),
     *	                    X2XRCD_REC(X2XRCD_INDEX(J)),
     *	                J+1,X2XRCD_FIELD(J+1),
     *	                    X2XRCD_REC(X2XRCD_INDEX(J+1))
	    IF(PRTFLG)
     *	      WRITE(6,9010) J,X2XRCD_FIELD(J),
     *	                    X2XRCD_REC(X2XRCD_INDEX(J)),
     *	                J+1,X2XRCD_FIELD(J+1),
     *	                    X2XRCD_REC(X2XRCD_INDEX(J+1))
	  ENDIF
...
C
	WRITE(5,*)
        WRITE (5,9022)                                                     !V03
	CALL INPNUM(PROMPT,FIELD,2,X2XRCD_ENTRIES-1,EXT)
	IF(EXT.EQ.-2) THEN
	  PRTFLG=.TRUE.
	  OPEN(6, FILE='X2RCDMOD.REP', STATUS='NEW', DISP='PRINT/DELETE')
	  IF(ST.NE.0) THEN
...
C
110	CONTINUE
	BOT=X2XRCD_RANGE(1,FIELD)
	TOP=X2XRCD_RANGE(2,FIELD)
	IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	  ENDOFF=0
	  IF(X2XRCD_VALUE(1,FIELD).NE.-1) THEN
	    DO 112 I=1,15
	      IF(X2XRCD_VALUE(I,FIELD).NE.-1) THEN
	        ENDOFF=I
	      ENDIF
112	    CONTINUE
	    WRITE(5,9025) (X2XRCD_VALUE(I,FIELD),I=1,ENDOFF)
	  ENDIF
C
C PROMPT FOR INPUT DATA.
C
	  WRITE (PROMPT2,9030) X2XRCD_FIELD(FIELD),BOT,TOP
...
C
	  IF(X2XRCD_VALUE(1,FIELD).NE.-1) THEN
	    VALID=.FALSE.
	    DO 120 J=1,15
	      IF(TEMP.EQ.X2XRCD_VALUE(J,FIELD)) VALID=.TRUE.
120	    CONTINUE
	    IF(.NOT.VALID) THEN
	      WRITE(5,9070) CHAR(7)
	      GOTO 110
	    ENDIF
	  ENDIF
	  X2XRCD_REC(X2XRCD_INDEX(FIELD))=TEMP
C
C CHARACTER FIELD.
C
	ELSE IF(BOT.EQ.-1) THEN
	  WRITE (PROMPT2,9032) X2XRCD_FIELD(FIELD), TOP
	  CALL WIMG(5,PROMPT2)
	  READ(5,9060) CHRSTR
...
C
	...
	CALL X2BSET(X2XRCD_BITMAP,FIELD,XRCD,REC)			    !V02
	UPDATE=.TRUE.
	CALL CLRSCR(5)
	WRITE(5,9000)
	GOTO 50
...
9000	FORMAT(//,T26,'GTECH Distributed Network',/,
     *	          T24,'Modify Statistics Report Code',//)

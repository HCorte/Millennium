...
C V03 10-DEC-1996 HXK Fixed option flag (now two bytes)
C V02 10-FEB-1992 JPJ ADDED (GVT)
C V01 13-NOV-1991 JPJ RELEASED FOR VAX (INSTANTS)
C
C SUBROUTINE TO DECODE INSTANT ORDER CONFIRMATION MESSAGE FROM TERMINAL
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE DICNF(TERMES,TRABUF,MESLEN)
	IMPLICIT NONE
C
...
C
        ...
        ENDIF
CC
C SET FIRST INVOICE NUMBER TO ZERO
C
        TRABUF(TIINV1) = 0
C
C GET 2ND AND 3RD INVOICE NUMBER
C
	CALL TERM_TO_HOST(TERMES(IND),TRABUF(TIINV2),2)
	IND=IND+2
C
	CALL TERM_TO_HOST(TERMES(IND),TRABUF(TIINV3),4)
	IND=IND+4
C

C
C SUBROUTINE X2RCLADD
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2RCLADD.FOV                                 $
C  $Date::   17 Apr 1996 16:28:18                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C This program will add a Report class record.
C
C Calling sequence:
C
C     CALL X2RCLADD(REC)
...
C
C
	DO 100 I=2,X2XRCL_ENTRIES-1
110	  CONTINUE
	  BOT=X2XRCL_RANGE(1,I)
	  TOP=X2XRCL_RANGE(2,I)
	  IF(BOT.GE.0) THEN
C
C DISPLAY SPECIFIC VALUES (IF THEY EXIST).
C
	    ENDOFF=0
	    IF(X2XRCL_VALUE(1,I).NE.-1) THEN
	      DO 112 J=1,15
	        IF(X2XRCL_VALUE(J,I).NE.-1) THEN
	          ENDOFF=J
	        ENDIF
112	      CONTINUE
	      WRITE(5,9025) (X2XRCL_VALUE(J,I),J=1,ENDOFF)
	    ENDIF
C
C PROMPT FOR INPUT DATA.
C
	    WRITE (PROMPT2,9030) X2XRCL_FIELD(I),BOT,TOP
	    CALL INPNUM(PROMPT2,X2XRCL_REC(X2XRCL_INDEX(I)),
     *	                BOT,TOP,EXT)
	    IF(EXT.EQ.-9) THEN
...
C
	    IF(X2XRCL_VALUE(1,I).NE.-1) THEN
	      VALID=.FALSE.
	      DO 120 J=1,15
	        IF(X2XRCL_REC(X2XRCL_INDEX(I)).EQ.X2XRCL_VALUE(J,I))
     *	          VALID=.TRUE.
120	      CONTINUE
	      IF(.NOT.VALID) THEN
	        WRITE(5,9070) CHAR(7)
...
C
	  ELSE IF(BOT.EQ.-1) THEN
	    WRITE (PROMPT2,9032) X2XRCL_FIELD(I), TOP
	    CALL WIMG(5,PROMPT2)
	    READ(5,9060) CHRSTR
	    INDX=(X2XRCL_INDEX(I)-1)*4+1
	    DO 130 J=1,TOP
	      X2XRCL_CREC(INDX)=CHRSTR(J)
	      INDX=INDX+1
130	    CONTINUE
	  ENDIF
100	CONTINUE
...
C
	...
	X2XRCL_CLASS=REC
CV02	X2XRCL_BITMAP=-1
	CALL X2BSET(X2XRCL_BITMAP,0,XRCL,REC)				    !V02
C
C UPDATE THE FILE.
C
8000	CONTINUE

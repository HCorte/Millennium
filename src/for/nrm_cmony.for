...
C Calling sequence:
C
C     STRING=CMONY(AMOUNT,LEN,FACTOR)
C
C Input parametrs:
C
...
C Output parameters:
C
C     CMONY      Char*32       !THE STRING TO BE PRINTED ON REPORTS
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	CHARACTER FUNCTION CMONY*32(AMOUNT,LEN,FACTOR)
	IMPLICIT NONE
...
C
	...
	ENDIF
        IF (IAND(AMOUNT,HMASK) .NE. 0) THEN
          ...
          BIG=(SBIT+DFLOAT(TEMP))*MON_UNIT/DOLL_BASE
        ELSE

...
C---- Local variables used.
	...
	CHARACTER*20 PWORD,CDSTPFN
	EQUIVALENCE  (PASS,PWORD)
	CHARACTER*2  FUN
	INTEGER*4    NAME(STRENM_LEN/4),ENAME(STRENM_LEN/4)
	INTEGER*4    DNAME(STRDES_LEN/4)
	CHARACTER    INPFIL,ANS
	CHARACTER    CENAME(STRENM_LEN),CNAME(STRENM_LEN)
	CHARACTER    CDNAME(STRDES_LEN)
	BYTE         BENAME(STRENM_LEN),BNAME(STRENM_LEN)
	BYTE         BDNAME(STRDES_LEN)
	INTEGER*4    TEMP		!used to restore to old values
	EQUIVALENCE  (DSTPFN,CDSTPFN)
...
C---- Initialize / set variables 
	...
	WRITE(DBINT_NAME,990) GTNAMES(TSTR)

	DESCRIPT_LINES = STRDES_LEN/28		!28 Bytes / line

C---- Check if super TRIPLE game is active

	GNUM = SCFGTN(TSTR,GIND)

	IF (GNUM .LT. 1) THEN
	  WRITE(5,991) IAM(),GTNAMES(TSTR),GIND
	  CALL XWAIT(2,2,ST)
	  RETURN
	END IF
...
C	
	PREV3 = 0
	IF(DRW.GT.1) THEN
	  CALL READW(INPFDB,DRW-1,DSTREC,ST)
	  ...
	  IF (ST .NE. 0) THEN
	    ...
	    GOTO 20
	  END IF
	  CALL ILBYTE(PREV3,DSTREV,2)          !GET PREVIOUS TEXT REV #
       ENDIF
...
30	CONTINUE
	...
	IF (FUN .EQ. 'MO' .AND. DSTSTS .GE. GAMOPN) THEN
	  CALL CLRSCR(5) 
	  WRITE(5,992) IAM(),GTNAMES(TSTR),GIND,DRW,IAM()
	...
	DO 80 J = 1,MAXSTRRW
	  DO 70 K = 1,STRNMS_LEN/4
	    IF (DSTNMS(K,J) .EQ. 0) DSTNMS(K,J) = '    '
70	  CONTINUE
80	CONTINUE


	DO 90 I = 1,STRENM_LEN/4
	  IF (DSTENM(I) .EQ. 0) DSTENM(I) = '    '
90	CONTINUE
...
105	CONTINUE
	...
	WRITE(5,903) GTNAMES(TSTR),GIND,DRW,WEK,GSTAT(LSTAT),
     *		     (DSTENM(K),K=1,STRENM_LEN/4)

	DSCIND = 0

	CALL FASTMOV(DSTDES,DNAME,STRDES_LEN/4)

	DO I = 1,DESCRIPT_LINES
	   WRITE(5,914) (CDNAME(K),K=DSCIND+1,DSCIND+28)
	   DSCIND = DSCIND + 28
	END DO
	...
	WRITE(5,905) DISTIM(DSTTIM),(DDATE(K),K=7,13),DDATE(5)
        WRITE(5,945) CMONY(DSTPRC,10,BETUNIT)

	WRITE(5,910) (DSTPFN(K),K=1,5), (DSTTVC(K),K=1,STRTVC_LEN/4)

	IF(DSTPCC.EQ.1) THEN
	    TYPE*,'*** NB! X-Y-17, 17-X-Y, 18-X-Y, X-18-Y, X-16-Y, X-Y-16 are NOT playable ***'
	ENDIF

	DO 110 I=BROW,EROW
	  WRITE(5,906) I,(DSTNMS(J,I),J=1,STRNMS_LEN/4)
110	CONTINUE

	IF (EROW .NE. MAXSTRRW) THEN
	  CALL WIMG(5,'Hit return to see remaining rows')
	  READ(5,901) ANS
	  BROW = EROW+1
	  EROW = EROW+19-DESCRIPT_LINES    
	  IF (EROW .GT. MAXSTRRW) EROW = MAXSTRRW
	  GOTO 105
	END IF
	IF (TSTFLG) THEN
	  WRITE(5,99999) IAM(),'REVISION:   ',DSTREV
	  WRITE(5,99998) IAM(),'DESCRIPT:   ',(DSTDES(K),K=1,STRDES_LEN/4)
	END IF
...
120	CONTINUE
	...
	WRITE(5,922) IAM(),DISTIM(DSTTIM)

	TEMP = DSTTIM

	CALL INPTIM('Enter time HH:MM:SS....................',DSTTIM,EXT)

	IF (EXT .NE. 0) DSTTIM = TEMP 

        WRITE(5,944) IAM(), CMONY(PRICE,10,BETUNIT)
        CALL INPMONY('Enter minimum stake.........',PRICE,BETUNIT,EXT)
        IF(EXT.NE.0) DSTPRC=PRICE

	WRITE(5,929) IAM()

	WRITE(5,930) IAM(),DSTPFN(1)

	WRITE (CDSTPFN,911) GIND,DRW

	CALL WIMG(5,'Enter pool file volume name............')

	READ(5,907) NAME(1)

	IF (NAME(1) .NE. '    ') DSTPFN(1)=NAME(1)

	WRITE(5,931) IAM()

	WRITE(5,932) IAM(),(DSTENM(K),K=1,STRENM_LEN/4)
...
C---- Did he/she hit return?
	...
	IF (CLEARED .EQ. STRENM_LEN/4) GOTO 140
...
140	CONTINUE
	...
	DO I = 1,DESCRIPT_LINES
	  WRITE(5,935) IAM(),I,(CDNAME(K),K=DSCIND+1,DSCIND+28)
	  DSCIND = DSCIND + 28
	END DO
	...
	DO I = 1,DESCRIPT_LINES
	  ...
	  READ(5,936) (CDNAME(K),K=DSCIND+1,DSCIND+28)
	  DSCIND = DSCIND + 28
	END DO
...
C---- Did he/she hit return?
        ...
        END DO
        IF (CLEARED .EQ. STRDES_LEN/4) THEN
           CLEARED = 0
           DO I = 1,STRDES_LEN/4
              IF (DSTDES(I) .EQ. 0) CLEARED = CLEARED + 1
           END DO
           IF (CLEARED .EQ. STRDES_LEN/4) THEN
              DO I = 1,STRDES_LEN/4
                 DSTDES(I) = '    '
...
165	CONTINUE
	...
	WRITE(5,938) IAM(),(DSTTVC(K),K=1,STRTVC_LEN/4)

	CALL WIMG(5,'Enter TV-Chanel Name...................')

	READ(5,939) (NAME(K),K=1,STRTVC_LEN/4) 
...
C---- Did he/she hit return?
	...
	IF (CLEARED .EQ. STRTVC_LEN/4) GOTO 300
...
300	CONTINUE
	...
	WRITE(5,941) IAM(),(DSTNMS(K,ROW),K=1,STRNMS_LEN/4)

	CALL WIMG(5,'Enter Team Name........................')

	READ(5,942) (NAME(K),K=1,STRNMS_LEN/4)
...
C---- Did he/she hit return?
	...
	IF (CLEARED .EQ. STRNMS_LEN/4) GOTO 210
...
C
	IF(TEMP.EQ.18) THEN
	  IF(DSTPCC.EQ.0) THEN
	    TYPE*,IAM(),'X-Y-17, 17-X-Y, 18-X-Y, X-18-Y, X-16-Y, X-Y-16 are playable'
	  ELSEIF(DSTPCC.EQ.1) THEN
	    TYPE*,IAM(),'X-Y-17, 17-X-Y, 18-X-Y, X-18-Y, X-16-Y, X-Y-16 are NOT playable'
	  ENDIF
	  CALL INPYESNO('Do you want to disable these combinations [Y/N] ?',
     *                   FLAG)
	  IF(FLAG.EQ.1) THEN
	    DSTPCC = 1
	  ELSE
...
C---- Load up player names.
	...
	DO I = 1,MAXSTRRW                     !FOR ALL ROWS
	   IF (DSTNMS(1,I) .NE. '    ') THEN
	     CALL MOVBYT(DSTNMS(1,I),
     *			 1,
     *			 BYTTAB,
     *			 BUFIND,
     *			 STRNMS_LEN-2)
	     BUFIND = BUFIND + (STRNMS_LEN-2)
	   END IF
	END DO
...
1020	CONTINUE
	...
	DSTSTS = GAMOPN
	DO 1030 I = 1,MAXSTRRW
	  IF (DSTSTA(I) .EQ. GAMINF) DSTSTA(I) = GAMOPN
1030	CONTINUE

	DO 1040 I = DSTBSD,DSTESD
	  CALL READW(DFDB,I,DAFREC,ST)
	  IF (ST .NE. 0) THEN
	    CALL FILERR(SCFSFN(1,DAF),2,ST,I)
	    GOTO 10
...
2010	CONTINUE
	...
	DSTSTS = GAMINF
	DO 2020 I = 1,MAXSTRRW
	  IF (DSTSTA(I) .EQ. GAMOPN) DSTSTA(I) = GAMINF
2020	CONTINUE

	DO 2030 I = DSTBSD,DSTESD
	  CALL READW(DFDB,I,DAFREC,ST)
	  IF (ST .NE. 0) THEN
	    CALL FILERR(SCFSFN(1,DAF),2,ST,I)
	    GOTO 10
...
3000	CONTINUE
	...
	IF ((FUN .EQ. 'CH') .AND. (DSTSTS .GE. GAMOPN)) THEN
	  CALL CLRSCR(5)
          TYPE*,IAM(),DBINT_NAME,GIND,' EVENT ',DRW,
	...
	CALL STRCOMP(FDB,VFDB,GIND,DRW,GNUM,ST)
...
906	FORMAT(1X,I2.2,1X,<STRNMS_LEN/4>A4)

907	FORMAT(A4)

908	FORMAT(1X,7A2,' has an invalid day status ')

909	FORMAT(1X,7A2,' is already active for Super Triple',
     *	       I1,' event ',I4)

910	FORMAT(1X,'Pool file name ',5A4,' TV-Chanel Name:',
     *	       <STRTVC_LEN/4>A4)

911	FORMAT(4X,':ST',I1,'P',I4.4,'.FIL   ')
...
932	FORMAT(1X,A,'(RETURN no change /+ clear)',<STRENM_LEN/4>A4)

933	FORMAT(<STRENM_LEN>A4)
...
99998   FORMAT(1X,A,<STRDES_LEN/4>A4)

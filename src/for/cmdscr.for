...
C SUBROUTINE CMDSCR
C
C V05 04-OCT-2000 UXN  GLOBAL RFSS #91. CLRSUM ADDED.
C V04 28-JUL-1998 RXK  Avoided to get 0 for revision # (byte 0) if updating
C V03 20-FEB-2000 HXK  Fix for rev#
C V02 14-JUL-1992 GCAN ADDED CLOSING TIME AND DATE CHANGE COMMANDS.
C V01 01-AUG-1990 XXX  RELEASED FOR VAX
C
C SUBROUTINE TO PROCESS SCORE GAME COMMANDS
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
	...
	GOTO 1000
C
C CHANGE SCORE GAME STATUS
C
10	CONTINUE
	GIND=TRABUF(TCMDT1)
	TRABUF(TCMOLD)=SCRSTS(GIND)
	SCRSTS(GIND)=TRABUF(TCMNEW)
	IF(TRABUF(TCMNEW).EQ.GAMBFD) THEN
	  SCRCTM(GIND)=TRABUF(TTIM)
	  CALL BSET(SCRTIM(GIND),1)
	  CALL CLRSUM
	ENDIF
	TEMP=SCRREV(GIND)
	I2TEMP(1)=I2TEMP(1)+7
...
20	CONTINUE
        GIND = TRABUF(TCMDT1)
        TRABUF(TCMOLD) = SCRTIM(GIND)
        SCRTIM(GIND) = TRABUF(TCMNEW)
        MESS(2) = TECMD
        MESS(3) = 12
        MESS(6) = GIND
        MESS(9) = 0

C SUBROUTINE DWIN_WININT
C
C V09 03-JUL-2000 UXN LDBLAT added.
...
C V03 10-NOV-1997 UXN WINREP_AUTO,BKKREP_AUTO ADDED FOR 
C		      AUTOMATED WINRPT,BKKREP
...
C=======OPTIONS /CHECK=NOOVERFLOW
	...
	SUBROUTINE DWIN_WININT(FILES,FILCNT)
...
C-------------------- Start of code -----------------------------------
	...
	CALL FASTSET(-1000,LDBDRW,NUMDBL)

	FILCNT = 0
	WVOL = P(ODD_DRWPCK)

	DO 100 GIND = 1,NUMDBL
     ...
     *            (DRWSTS(MLWININD,GNUM).NE.WINPRV)) DRAW=0
	   ...
	   WRITE(6,930) IAM(),GTNAMES(TDBL),GIND,DRAW
	   ...
	   CALL LOGGAM(TDBL,GIND,DDBREC,LDBSTS)

	   IF (LDBSTS(GIND) .EQ. GAMCAN .OR.
     *         LDBSTS(GIND) .EQ. GAMREF) THEN
	      DBREFUND(GIND) = .TRUE.
	   ELSE
	      DO 105 I=1,MAXDBLRW
		 IF (LDBSTA(I,GIND) .EQ. GAMCAN .OR.
     *		     LDBSTA(I,GIND) .EQ. GAMREF) THEN
		    DBREFUND(GIND) = .TRUE.
		    WRITE(6,910) IAM(),GTNAMES(TDBL),GIND,I
		 END IF
105	      CONTINUE
	   END IF

	   IF ( LDBSTS(GIND) .LT. GAMENV .OR.
     *	        LDBSTS(GIND) .EQ. GFINAL .OR.
     *	        LDBSTS(GIND) .EQ. GAMREF) THEN
	     WRITE(6,920) IAM(),GTNAMES(TDBL),GIND,LDBSTS(GIND),GAMENV
	     CALL GPAUSE
	   END IF
	   
	   IF(LDBSTS(GIND).EQ.GAMREF) LDBSTS(GIND) = GAMCAN

	   IF(LDBLAT(LATCDC,GIND).GT.0) DBREFUND(GIND) = .TRUE.
	   LDBLAT(LATCNT,GIND) = 0
	   LDBLAT(LATAMT,GIND) = 0
	   ...
	   LDBTAX(GIND) = 0
           ...
           LDBREF(GIND)   = 0
           LDBWON(GIND)   = 0
           CALL FASTSET(0,LDBWCP(1,GIND),MAXDBLTI)
           CALL FASTSET(0,LDBWBT(1,1,GIND),NUMTOT*MAXDBLTI)
           CALL FASTSET(0,LDBWRA(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LDBWPR(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LDBWRO(1,1,GIND),NUMTOT*2)
           CALL FASTSET(0,LDBWPA(1,1,GIND),NUMTOT*2)

           CALL FASTSET(0,LDBWPO(1,1,GIND),NUMTOT*2)
	   CALL FASTSET(0,LDBSBR(1,GIND),MAXDBLRW)
	   CALL FASTSET(0,LDBHST(1,GIND),MAXDBLTI)
	   IF(WVOL.EQ.0) THEN
	   ...
	   DO 90 CDC = LDBBSD(GIND),LDBESD(GIND)

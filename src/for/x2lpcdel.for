C
C SUBROUTINE X2LPCDEL
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2LPCDEL.FOV                                 $
C  $Date::   17 Apr 1996 16:22:42                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C This program will mark a record in the Local Port Configuration
C file as deleted.  This is done by negating the record index number.
C
C Calling sequence:
C
C     CALL X2LPCDEL(REC)
...
C
	IF(ANS.EQ.1) THEN
	  ...
	  X2XLPC_REC(1)=0
CV02	  CALL BSET(X2XLPC_BITMAP,1)
...
C
	IF(UPDATE) THEN
	  ...
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XLPC),'WRITEW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  WRITE(5,9080) REC

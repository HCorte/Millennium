C GUICMD_001.FOR
C
C V03 16-FEB-2001 UXN CHKTIM,FPTTIM unit changed to minutes.
C V02 09-FEB-2001 UXN PRMSTR added.
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE GUICMD_001(OUTBUF,MES_LEN,RET_CODE)
	IMPLICIT NONE
C
...
C
	...
	IF(PARM(PAR_ID,PAR).EQ.DISKSW) THEN
	    STATUS_STR = 'DISKSW parameter change disabled'
	    STATUS = 1
	    GOTO 100
	ENDIF

	VAL = GUI_ARGVAL(2)
	IF(PARM(PAR_ID,PAR).NE.ROMREV.AND.PARM(PAR_ID,PAR).NE.GVTREV) THEN 
	...
	ENDIF
C
	IF(PARM(PAR_ID,PAR).EQ.CHKTIM) THEN
	   VAL = VAL * 60 * 8 ! in 125 msec
	ELSEIF(PARM(PAR_ID,PAR).EQ.FPTTIM) THEN
	   VAL = VAL * 60
	ENDIF
C
C Build the command

C
C SUBROUTINE X2VISGVT
C  
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2VISGVT.FOV                                 $
C  $Date::   17 Apr 1996 16:40:14                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
C**************************** END X2X PVCS HEADER *****************************
C
C ** Source - x2visgvt.for **
C
C V02 07-OCT-94 SCD MAKE XGVT AND XSTN BITS MUTUALLY EXCLUSIVE
C V01 06-OCT-94 SCD CREATE ROUTINE TO HANDLE GVT IDs
C
C ===========================================================
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2VISGVT(SCRIDX,LEVEL,GVTID,LEN,ERR)
	IMPLICIT NONE
C
...
C
	CHARACTER   GVTID*(*)         !GVT ID in ascii		!V02
        CHARACTER   NULL_X2XGVT*(LGSER) ! ALL 0'S.		!V02
...
C
	IF(GVTID.GT.NULL_X2XGVT) THEN
	  CALL ATOH(GVTID,1,MIN0(LEN,LGSER),EVSNNUM,ERR)
	  IF(ERR.NE.0) RETURN
...
C
	  IF(STN.NE.-1) THEN
	    ...
	    X2SCRN(X2SCRN_KEYLEV3+LEVEL,SCRIDX)=XGVTIDX
C
...
C	STATION, IDU or ADR bits are set, then clear them.

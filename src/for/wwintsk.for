...
C V04 11 Jan 1994 HXK ADDED WINRPT.
C V03 21 Jan 1993 DAB Initial Release
C  		      Based on Netherlands Bible, 12/92, and Comm 1/93 update
C  		      DEC Baseline
C V02 12-NOV-1991 GCAN INITIAL RELEASE FOR NETHERLANDS
C V01 01-AUG-1990 XXX  RELEASED FOR VAX
C
C
C WINNERS TIP WINNER SELECTION PROCEDURE CONTROL TASK
C
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
	...
	CALL CLOSEFIL(FDB)
	CALL PRMYESNO(
     *   'Are you sure you want Winners Tip winner selection [Y/N]? ',
     *   FLAG)
...
C
	...
	ENDIF
C
C LOAD AND START WWINSEL TASK
C
	WRITE(5,901) IAM(),'WWINSEL  '
	CALL RUNTSK(8HWWINSEL )
	WRITE(5,902) IAM()
C
...
C
900	FORMAT(1X,A,' Mount drawing packs and continue WWINTSK ')
901	FORMAT(1X,A,' Begining execution of ',A8)
902	FORMAT(1X,A,' Winners Tip winner selection complete ')

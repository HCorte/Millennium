...
C FUNCTION CMONYI8
C
C V02 23-NOV-2000 UXN CENT_LEN == 0 changes.
C V01 08-MAR-1991 JPJ INITIAL RELEASE FOR MARYLAND
C
C THIS CHARACTER FUNCTION
C
C Calling sequence:
C
C     STRING=CMONYI8(AMOUNT,LEN,FACTOR)
C
C Input parametrs:
C
C     AMOUNT     Int*4         !THE AMOUNT OF MONEY IN UNITS WE USE
C     AMOUNT(2)  Int*4         !AMOUNT LEFT AFTER UNITS
C     LEN        Int*4         !THE LENGTH OF THE STRING RETURNED
C     FACTOR     Int*4         !SHOWS WHICH VALUE OF ONE UNIT TO USE
C
C Output parameters:
C
C     CMONYI8    Char*32       !THE STRING TO BE PRINTED ON REPORTS
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C
	...
	ENDIF
        IF (IAND(AMOUNT(1),HMASK) .NE. 0) THEN
          TEMP =IAND(AMOUNT(1),LMASK)
          TEMP2=IAND(AMOUNT(2),LMASK)
          BIG=((SBIT+DFLOAT(TEMP))*MON_UNIT+DFLOAT(TEMP2))/DOLL_BASE
        ELSE
          BIG=(DFLOAT(AMOUNT(1))*MON_UNIT+DFLOAT(AMOUNT(2)))/DOLL_BASE
        END IF

C
C SUBROUTINE FRMCCONF
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]FRMCCONF.FOV                                 $
C  $Date::   17 Apr 1996 13:13:18                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - frmcconf.for;1 **
C
C FRMCCONF.FOR
C
C V01 28-NOV-90 XXX RELEASED FOR VAX
C
C CALL FRMCCONF(SSAP,DSAP,BUF)
C
C IN:
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE FRMCCONF(SSAP,DSAP,BUF)
	IMPLICIT NONE
C
...
C
	IF(LANTEST.NE.0) THEN
	   CALL OPS('**** CON CONF ****',CONN,LAN)
	   IF(SSAP.LE.0.OR.SSAP.GT.MAXSAP.OR.
     *	      DSAP.LE.0.OR.DSAP.GT.MAXSAP) THEN
	      TYPE*,'**** FRMCCONF:... ILLEGAL SAP ****[',SSAP,DSAP,']'
	      GOTO 200
	   ENDIF
	   IF(CONN.LE.0.OR.CONN.GT.MAXCON) THEN
            TYPE*,'**** FRMCCONF:... ILLEGAL CONNECTION ****[',CONN,']'
	      GOTO 200
	   ENDIF
	   IF(BUF.LE.0.OR.BUF.GT.LANBNUM) THEN
           TYPE*,'**** FRMCCONF:... ILLEGAL BUF ****[',BUF,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	   IF(LAN.LE.0.OR.LAN.GT.MAXLAN) THEN
           TYPE*,'**** FRMCCONF:... ILLEGAL LAN ****[',LAN,SSAP,DSAP,']'
	     GOTO 200
	   ENDIF
	ENDIF
C
...
C
	IF(LANSAPSTS(SSAP).NE.SAPUP) THEN
...
C
	IF(LOCLAN(SSAP,LAN).EQ.LANUP) THEN
	   ...
	   CALL I4TOBUF2(FRMLEN(FRCCONF),LANBUF(1,BUF),TYPEBEG-1)
	   CALL ISBYTE(DSAP,LANBUF(1,BUF),DSAPBEG-1)
	   CALL ISBYTE(SSAP,LANBUF(1,BUF),SSAPBEG-1)
	   CALL ISBYTE(CTRLUI,LANBUF(1,BUF),CTRLBEG-1)
	   CALL ISBYTE(FRCCONF,LANBUF(1,BUF),FRTYPBEG-1)
	   CALL ISBYTE(LAN,LANBUF(1,BUF),TLANBEG-1)
	   CALL ISBYTE(LANCONN(CONN),LANBUF(1,BUF),LANDATAB-1)
	   CALL ISBYTE(LANMODE(SSAP),LANBUF(1,BUF),LANDATAB)
	   CALL ISBYTE(LANTYPE(SSAP),LANBUF(1,BUF),LANDATAB+1)
	   CALL GOWRITE(BUF)
	   BUF=0
	ENDIF
C

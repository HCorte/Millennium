C
C SUBROUTINE WLOG
C $Log:   GXAFXT:[GOLS]WLOG.FOV  $
C  
C     Rev 1.0   17 Apr 1996 16:03:02   HXK
C  Release of Finland for X.25, Telephone Betting, Instant Pass Thru Phase 1
C  
C     Rev 1.0   21 Jan 1993 18:07:26   DAB
C  Initial Release
...
C WLOG.FOR
C
c V03 08-JAN-93 TD  COMMENTED-OUT THE NWLOG ENTRY POINT BECAUSE NO OTHER
C		    ROUTINE CALLS NWLOG
C V02 25-JAN-91 TKO DO NOT KICK LOGGER WHEN BUFFER FULL
C	            DO NOT SET TASKMAP FLAGS WHEN READING
C	            DO NOT USE HOLD WHEN READING - USE WAIT
...
C
C CALLING SEQUENCE:
C     CALL WLOG(SERIAL,RECORD,TASK)
C     CALL NWLOG(SERIAL,RECORD,TASK)
C INPUT
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE WLOG(SERIAL,RECORD,TASK)
	IMPLICIT NONE
C
	...
	INTEGER*4 RECORD(LREC*3), BLOCK, LENGTH, TYPE, CNT
	INTEGER*4 K, RTYPE, BL, ST, UPCNT, OFFSET, INDEX
	INTEGER*4 TASK, SERIAL
...
C
CCC	ENTRY NWLOG(SERIAL,RECORD,TASK)
CCC	WFLAG=.FALSE.
CCC	GOTO 5
C
C
...
C
C*V02*	      LOGBUF(TSKMAP,BL)=IOR(LOGBUF(TSKMAP,BL),LOGID(TASK))
C
C ALWAYS EXIT CRITICAL REGION
C
	      CALL LOKOFF(P(LOGFLG))
...
C BECOMES AVAILABLE.
C
C*V02*	LOGBUF(TSKMAP,1)=IOR(LOGBUF(TSKMAP,1),LOGID(TASK))
C
C ALWAYS EXIT CRITICAL REGION
C
	CALL LOKOFF(P(LOGFLG))

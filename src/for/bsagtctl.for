...
C
C
C V05 24-JAN-2011 RXK 0X REMOVED FROM FORMATS
C V04 FRP Modify for EM Joker (17-APR-2009)
...
C
C INFORMATION ABOUT JOKER WITH EM MUST BE UPDATED AS TOTJOK (TRATED AS ADD-ON)
C
      IF(TRABUF(TGAM) .EQ. 5) GOTO 1000
C
...
C 
      ENDDO
C
C CALCULATE PARTIAL TOTALS FOR SIMPLE WAGERS ( ALSO FOR JOKER GAME WITH EM)
C
      CNTWAG = 1
      CALL SEARCH_WAGER_INDEX_POSITION(CNTWAG, IDXPOS)
      CALL ADD_TOTALS(TOTSIMWAG, MFCWSB, TOTJOK, WEEK1, JOKGAM, IDXPOS, 1)
      CALL ADD_TOTALS(TOTSIMWAG, MFCWSB, TOTJOK, WEEK5, JOKGAM, IDXPOS, 1)
C
...
C
C CALCULATE PARTIAL TOTALS FOR MULTPLE WAGERS ( FOR JOKER GAME WITH EM)
C
      CNTWAG = 1
      CALL SEARCH_WAGER_INDEX_POSITION(CNTWAG, IDXPOS)
      CALL ADD_TOTALS(TOTMULWAG, MFCWMB, TOTJOK, WEEK1, JOKGAM, IDXPOS, 1)
      CALL ADD_TOTALS(TOTMULWAG, MFCWMB, TOTJOK, WEEK5, JOKGAM, IDXPOS, 1)
C
...
C
      ...
      INTEGER * 4 JOKEMTOTAL                            ! JOKER WITH EM TOTAL
...
C
C CALCULATE JOKER WITH EM GRAN TOTAL
C
      JOKEMTOTAL = GRDTOT(TOTJOK, WEEK1, JOKGAM) + 
     *             GRDTOT(TOTJOK, WEEK5, JOKGAM)
C
...
C
      WRITE(REPIDFIL, 400)
C
C PRINT JOKER WITH EM
C
      WRITE(REPIDFIL, 450) JOKEMTOTAL
C
...
400   FORMAT(X)
C
450   FORMAT(X, 10X, 'APOSTAS JOKER EUROMILHOES', 5('.'), X, I12)
C

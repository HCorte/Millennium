C
C SUBROUTINE X2CLOSE
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2CLOSE.FOV                                  $
C  $Date::   17 Apr 1996 16:13:28                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
	IF (IAND(X2X_DEBUG,X2X_DEBUG_SUBS).NE.0) TYPE *,IAM(),'X2CLOSE'
20	CONTINUE
...
30	CONTINUE
	...
	IF (ST.NE.2) GOTO 30
	IF (IAND(X2X_DEBUG,X2X_DEBUG_SUBS).NE.0) TYPE*,'RETURN X2CLOSE'
	RETURN
	END

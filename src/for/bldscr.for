...
C V02 09-JAN-1994 HXK ADDED MDS TO SCORE.
C V01 30-JUN-1993 GXA Initial revision.
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C		  score game with ability to 'undo'
C
C	input   : FILE   file name of score game file
...
C
	INTEGER*4 CDSCTIM	!closing time
	INTEGER*4 CDSCPRC	!base price
	INTEGER*4 CDSCSPR	!pool percentage
C
...
C
	...
	CALL READW(FDB,DRAW,DSCREC,STATUS)
...
10	CALL CLRSCR(5)
	WRITE(5,700) GNAME
	WRITE(5,710) DISTIM(CDSCTIM)
	WRITE(5,720) CMONY(CDSCPRC,10,BETUNIT)
	WRITE(5,730) DISPER(CDSCSPR)

        DO I = 2, MAXMLTD_AVL
            IF(DSCMDS(I).NE.0) THEN
                WRITE(5,9190) I
            ENDIF
        ENDDO
...
510	CONTINUE
	...
	CALL WRITEW(FDB,DRAW,DSCREC,STATUS)
	IF(STATUS.EQ.144) THEN

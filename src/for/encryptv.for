C
C SUBROUTINE ENCRYPTV
C
C ENCRYPTV.FTN                                                                  
C 
C V02 28-FEB-2001 UXN GLOBAL RFSS #191
C V01 15-SEP-1990 WOL INITIAL RELEASE FOR MARYLAND.                    
C                                                                               
C This subroutine will encrypt the buffer using encryption key                  
C                                                                               
C Calling sequence:                                                             
C                                                                               
C     CALL ENCRYPTV(BUFFER, CODEKEY,HALFCODEKEY)                                
C                                                                               
C Input parameters:                                                             
C                                                                               
C     BUFFER      Int*2(LENBUF)   Buffer to be encrypted                        
...
C Output parameters:                                                            
C                                                                               
C     BUFFER      Int*2(LENBUF)   Encrypted buffer                              
C                                                                               
C                                                                               
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++            
...
C=======OPTIONS /CHECK=NOOVERFLOW
      SUBROUTINE ENCRYPTV(BUFFER,CODEKEY,HALFCODEKEY)                           
      IMPLICIT NONE                                                     
C                                                                               
      ...
      INTEGER*4   TWOI4(2)                                               
      INTEGER*4   K, MYBYT1, MYBYT2, MYBYT3, MYBYT4                          
      INTEGER*2   I2TEMP
      BYTE        I1TEMP(2)
      BYTE        BYT1(4), BYT2(4)
...
C
C	POINT TO OFFSET OF DATA IN THE BUFFER AND CALCULATE LENGTH
C
      STARTOFF = (INPTAB*2 )*2 - 1                                              
      I1TEMP(1)=BUFFER(OUTLEN*2-1)
      I1TEMP(2)=BUFFER(OUTLEN*2)
      BUFFLEN = I2TEMP - 2
      IF (BUFFLEN .LT. 8) BUFFLEN = 8                                           
      BUFFOFF = STARTOFF                                                        
C
C	DETERMINE SHIFTS TO USE FOR ROTATEL STEP
C
      MYBYT1=CODEKEY(1)
      MYBYT2=CODEKEY(2)
      DO 2210 K = 1, 4                                                          
...
C                                                                               
C ROTATE THE BYTES OF TWOI4 TO THE LEFT                                         
C                                                                               
      TEMP=TWOI4(1)      
C
...
C	
        M1=ISHFT(TEMP,CNT)
        M2=ISHFT(TEMP,CNT-32)
        TEMP=M1+M2
C	
C	SHIFTS CYCLICALLY LAST BYTE
C
        IF(CNT .EQ. 8) GO TO 2260
        M1=ZEXT(BTEMP(1))
        M2=ISHFT(M1,CNT)
        M3=IAND(M2,MASK)
        M2=ISHFT(M1,CNT-8)
...
C
      ...
      BUFFOFF = BUFFOFF + 8                                                     
      IF (BUFFOFF .GE. STARTOFF + BUFFLEN) GO TO 2500                           
      IF (BUFFOFF + 8 .GT. STARTOFF + BUFFLEN)                                  

C
C SUBROUTINE X2RESSTN
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2RESSTN.FOV                                 $
C  $Date::   17 Apr 1996 16:31:22                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C  DEC Baseline
C
C ** Source - x2resstn.for;1 **
C
C X2RESSTN.FOR
C
C V01 01-AUG-90 XXX RELEASED FOR VAX
C
C This subroutine will create a reset station data unit.
C NOTE:  This routine is only used when an error has occurred
C during the normal process of responding to station upline
C messages.  If the command/status code of the orginal message
C contains 255, then send a hard reset.
C
C Calling Sequence:
C
C     CALL X2RESSTN(TRABUF,MESS,ORGMESS,LEN)
C
C Input parameters:
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2RESSTN(TRABUF,MESS,ORGMESS,MESLEN)
	IMPLICIT NONE
C
...
C
     ...
     *	            X2STMES_DATATYPE-1)
C
C IF THE ORIGINAL MESSAGE COMMAND IS 255, SEND A HARD RESET.
C NOTE: THE 255 IN THE ORIGINAL MESSAGE IS SET BY CMDX2X FOR
C VISION COMMANDS.
C
	CALL ILBYTE(RESTYP,MESS,X2STMES_CODE-1)
	IF(RESTYP.EQ.X2STMES_RESET) THEN
	  CALL ISBYTE(X2STMES_RESET,MESS,X2STMES_CODE-1)
	ELSE
	  CALL ISBYTE(X2STMES_SOFT_RESET,MESS,X2STMES_CODE-1)
	ENDIF
C

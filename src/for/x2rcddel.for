C
C SUBROUTINE X2RCDDEL
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2RCDDEL.FOV                                 $
C  $Date::   17 Apr 1996 16:27:50                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C This program will mark a record in the Statistics Report file
...
50	CONTINUE
	...
	DO 100 I=1,LINES
	  OFFSET=(I-1)*2+1
C
C NOTE: OFFSET 4 IS HARDCODED TO ALLOW FIELD 8 (NAME)
...
C
	IF(ANS.EQ.1) THEN
	  ...
	  X2XRCD_REC(1)=0
CV02	  CALL BSET(X2XRCD_BITMAP,1)
...
C
	IF(UPDATE) THEN
	  ...
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XRCD),'WRITEW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  WRITE(5,9080) REC

...
C V10 02-FEB-1997 WPW Next pack and next game numbers changed.
C V09 28-JAN-1997 HXK IPS LOTGEN release
C V08 17-JAN-1997 HXK Removed series number
C V07 18-DEC-1996 HXK Removed commented out code
C V06 05-DEC-1996 HXK Updated for Finland IPS
...
C
C SUBROUTINE TO BUILD INSTANT INVENTORY REPORT OUTPUT MESSAGES.
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE OIINV(TRABUF,OUTTAB,OUTLEN,CRSBUF)
	IMPLICIT NONE
C
...
C MORE FLAG
C
	IF(CRSBUF(NXTGAM).NE.0.OR.CRSBUF(NXTPCK).NE.0) THEN
	  OUTTAB(IND+0) = '01'X
	ELSE
	  OUTTAB(IND+0) = '00'X
	ENDIF
...
C
	DO 100 I=0,CRSBUF(NBRENT)-1
	   ...
	   CALL HOST_TO_TERM(OUTTAB(IND), CRSBUF(INGAME+I), 2)
	   IND=IND+2
C
C LOT NUMBER
C
	   CALL HOST_TO_TERM(OUTTAB(IND), CRSBUF(INPACK+I), 4)
	   IND=IND+4
C
C STATUS
C
	   OUTTAB(IND+0)=CRSBUF(INSTAT+I)
	   IND=IND+1
100	CONTINUE
	OUTLEN=IND-1
C

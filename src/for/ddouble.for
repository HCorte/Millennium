C 
C SUBROUTINE DDOUBLE
C  
C V12 02-FEB-2000 UXN TNFRAC added.
C V11 28-DEC-1999 OXK Fixed game revision error setting.
C V10 01-JUN-1999 UXN Amounts are now 4 bytes.
C V09 16-MAR-1999 RXK Gtyp+gind 5+3 bits change.
C V08 21-MAY-1996 HXK Updates from Mike Pindrik, Siew Mun Wo for X.25 / TEBE
C V07 23-JAN-1996 HXK Changed price check
C V06 22-JAN-1996 HXK Fix for TWSYSN = 0
C V05 21-JAN-1996 HXK Added MAXDBL, MAXWIT, MAXCPL parameters
C V04 06-JAN-1996 HXK Fix for logging of Double / Couple
C V03 18-DEC-1995 HXK Made several fixes for system bets
C V02 15-DEC-1995 HXK Added extra checks, further fixes
C V01 23-NOV-1995 PXB Initial revision.
C
C SUBROUTINE TO DECODE SUPER DOUBLE WAGERS
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C=======OPTIONS /CHECK=NOOVERFLOW/EXT
	...
	SUBROUTINE DDOUBLE(TERMES,TRABUF,MESLEN)
...
C---- Get game index and system type.
	...
	GIND = ISHFT(TEMP,-4)
	TRABUF(TGAMIND) = GIND
	IF(GIND.LT.1 .OR. GIND.GT.NUMDBL) THEN
	  TRABUF(TERR) = SYNT
	  SYNTERRCOD = 4
	  RETURN
	ENDIF

 	TRABUF(TGAM) = GTNTAB(TDBL,GIND)
...
C---- Get Coupon ID.
	...
	TRABUF(TWDBCOUPID) = ZEXT(TERMES(IND))
	IND = IND + 1
...
C---- Get Super Double bet data.

	IF (TRABUF(TWSYST) .EQ. NOSYS) THEN
	  IND = IND + 2   ! what about #1st's, #2nd's
	  DO I = 0,TRABUF(TWNBET)-1
	     TRABUF(TWDBROW1+I*TWDBBLEN) = TERMES(IND)
             IND = IND + 1
          ENDDO
	  DO I = 0,TRABUF(TWNBET)-1
	     TRABUF(TWDBROW2+I*TWDBBLEN) = TERMES(IND)
             IND = IND + 1
	     IF (TRABUF(TWDBROW1+I*TWDBBLEN) .EQ.
     *		 TRABUF(TWDBROW2+I*TWDBBLEN)) THEN
               TRABUF(TERR) = SYNT
               SYNTERRCOD = 187
               RETURN
	     END IF
	  ...
	  DO I = 0,TRABUF(TWNBET)-1
	     ...
	     TEMP3 = ZEXT(TERMES(IND+2))
	     TEMP4 = ZEXT(TERMES(IND+3))
	     TRABUF(TWDBAMT+I*TWDBBLEN) = (ISHFT(TEMP1,24) + ISHFT(TEMP2,16)+
     *                                      ISHFT(TEMP3,8) + TEMP4)
	     IND = IND + 4
	  END DO
	ELSE
     ...
     *	      TRABUF(TSIZE).LE.1) THEN
          ...
          DO I = 0,MAX_MATCH-1
            IF (I .LE. NUM_FST-1) THEN
              TRABUF(TWDBROW1+I*TWDBBLEN) = TERMES(IND)
	      IF(DBLPCC(GIND).EQ.1) THEN
		IF(TERMES(IND).EQ.18) THEN
		    TRABUF(TERR) = SYNT
		    SYNTERRCOD = 83
		    RETURN  
		ENDIF
	      ENDIF
              IND = IND +1
            ELSE
              TRABUF(TWDBROW1+I*TWDBBLEN) = 'FF'X
            END IF
          END DO

          DUPLICATE_CNT = 0
          DO I = 0,MAX_MATCH-1
             IF (I .LE. NUM_2ND-1) THEN
               TRABUF(TWDBROW2+I*TWDBBLEN) = TERMES(IND)
	       IF(DBLPCC(GIND).EQ.1) THEN
		 IF(TERMES(IND).EQ.17) THEN
		    TRABUF(TERR) = SYNT
		    SYNTERRCOD = 84
		    RETURN  
		 ENDIF
	       ENDIF
               IND = IND + 1
               DO J = 0,NUM_FST-1
		  IF (TRABUF(TWDBROW1+J*TWDBBLEN) .EQ.
     *                TRABUF(TWDBROW2+I*TWDBBLEN)) THEN
                     DUPLICATE_CNT = DUPLICATE_CNT + 1
                  ENDIF
	       END DO
             ELSE
               TRABUF(TWDBROW2+I*TWDBBLEN) = 'FF'X
             END IF 
          END DO

          IF (TRABUF(TWSYSN) .NE. (NUM_FST * NUM_2ND) - DUPLICATE_CNT) THEN
            SYNTERRCOD = 83
            TRABUF(TERR)=SYNT
            RETURN
          END IF
          ...
          TEMP3 = ZEXT(TERMES(IND+2))
          TEMP4 = ZEXT(TERMES(IND+3))
          TRABUF(TWDBAMT) = (ISHFT(TEMP1,24) + ISHFT(TEMP2,16)+
     *                       ISHFT(TEMP3,8) + TEMP4)/TRABUF(TWSYSN)
	  IND = IND + 4

	ENDIF

C---- Check Super Double bat data.

	IF (TRABUF(TWSYST) .EQ. NOSYS) THEN
	   DO I = 0,TRABUF(TWNBET)-1
	      AMT = TRABUF(TWDBAMT+I*TWDBBLEN)
	      IF (AMT .LT. DBLPRC(GIND)) THEN
	         TRABUF(TERR) = SYNT
		 SYNTERRCOD = 190
		 RETURN
	      END IF
	      IF(AMT .GT. P(MAXDBL)) THEN
	         TRABUF(TERR) = SYNT
		 SYNTERRCOD = 191
		 RETURN
	      END IF
              ROW1 = TRABUF(TWDBROW1+I*TWDBBLEN)
              ROW2 = TRABUF(TWDBROW2+I*TWDBBLEN)
	      IF (ROW1 .GT. MAXDBLRW .OR. ROW2 .GT. MAXDBLRW) THEN
	        TRABUF(TERR) = SYNT
		SYNTERRCOD = 130
		RETURN
	      END IF
		...
		RETURN
	      END IF
              ...
              IF(DBLSTA(ROW1,GIND).NE.GAMOPN .OR.
     *           DBLSTA(ROW2,GIND).NE.GAMOPN) THEN
                 TRABUF(TERR) = GREV
       		 TRABUF(TSUBERR) = IOR(TRABUF(TSUBERR),GREV_GAMCLT)
                 SYNTERRCOD = 135
                 RETURN
	      ...
	      IF(DBLPCC(GIND).EQ.1) THEN
	         IF(ROW1.EQ.18.OR.ROW2.EQ.17) THEN
		    TRABUF(TERR) = SYNT	
		    SYNTERRCOD = 132
		    RETURN
		 ENDIF
	...
	ELSE
	   ...
	   DO I = 0,MAX_MATCH-1
		 ...
		 END IF
		    ...
		    RETURN
		 END IF
                 IF(DBLSTA(ROW1,GIND).NE.GAMOPN) THEN
                    TRABUF(TERR) = GREV
      		    TRABUF(TSUBERR) = IOR(TRABUF(TSUBERR),GREV_GAMCLT)
                    SYNTERRCOD = 145
                    RETURN
		 ...
		 END IF
		    ...
		    RETURN
		 END IF
                 IF(DBLSTA(ROW2,GIND).NE.GAMOPN) THEN
                    TRABUF(TERR) = GREV
                    TRABUF(TSUBERR) = IOR(TRABUF(TSUBERR),GREV_GAMCLT)
                    SYNTERRCOD = 155
                    RETURN
		 ...
		 DB2CNT = DB2CNT + 1
	      END IF
	   END DO
	   IF (DB1FFCNT .NE. 0 .AND. DB2FFCNT .NE. 0) THEN
	      TRABUF(TERR) = SYNT
	      SYNTERRCOD = 163
	      RETURN
	   END IF
	   NBETS = DB1CNT*DB2CNT - DUPLICATE_CNT
	   ...
	   IF(TRABUF(TWDBAMT) .GT. P(MAXDBL)) THEN
	      TRABUF(TERR) = SYNT
	      SYNTERRCOD = 180
	      RETURN
	   ENDIF

	   IF(TRABUF(TWDBAMT) .LT. DBLPRC(GIND)) THEN
	      TRABUF(TERR) = SYNT
	      SYNTERRCOD = 181
	      RETURN
	   ENDIF

	   IF(TRABUF(TWAMT) .LT. DBLPRC(GIND)) THEN
	      TRABUF(TERR) = SYNT
	      SYNTERRCOD = 182
	      RETURN
	   ENDIF

	   IF(TRABUF(TWAMT) .GT. P(MAXDBLS)) THEN
	      TRABUF(TERR) = SYNT
	      SYNTERRCOD = 183
	      RETURN
	   ENDIF

C
C GET_OFFPRIZES_ANT.FOR                                                                    
C
C V04 12-AUG-2011 RXK "Millennium" replaced with "ES Evolution"
C V02 24-DEC-2010 FRP Lotto2 Changes
C V01 13-MAR-2001 EPH INITIAL VERSION FOR SCML
C                                                                               
C READ OLD PAYMENT ORDERS SENT BY OFFLINE SYSTEM AND CREATE 
...
C     ******************************
	 ...
	 CALL DISPERR (MSG_LUN, 'Error getting Order Number from Configuration File', 0, ' ', 0, ' ', 0)
      ...
      ENDDO
      TYPE*,IAM(),' '
      TYPE*,IAM(),'>>> Updating OP numbers in SCF with OP numbering from SCML'
      LAST_OP_NUM(4) = LAST_OP_NUM(2)      !LOTO2 will follow LOTO numbering
      DO I=1,MAXGAM
         IF (LAST_OP_NUM(I).GT.ORDER_NUMBER(I)) THEN
            TYPE*,IAM(),'    Game =', I, '     SCF OP = ', ORDER_NUMBER(I)
            TYPE*,IAM(),'                           CHANGES TO', LAST_OP_NUM(I) 
            ORDER_NUMBER(I) = LAST_OP_NUM(I)
         ENDIF
      ENDDO
      TYPE*,IAM(),' '
...
C
C        SO... IF OFF-LINE HAS NOT NUMBERED... I DO IT
C
...
C      OPS_REC.GENERATION_CDC = DAYCDC
      IF (OOFWAGANT_REC.NUM_OP .EQ. 0) THEN
C
C	 Offline did not give this old prize an OP number, so it has not printed it yet
C
...
C
C     UPDATE ORDER SEQUENTIAL NUMBER INTO SCF
C     ****************************************    

...
!===============================================================================
        call copyrite
        type*,iam()
        type*,iam(),'<<<<<<<<<<<< GENCODS - CODfiles generation >>>>>>>>>>>>'
!       
10      continue
        type*,iam()     
...
!       
        ...
        year = mod(year,100)
!                
        emis = pasextdrw(week,mod(year,10),gind)
        if(emis.le.0) then
...
!        
        call inpnum('Enter initial serie to search, (A)ll or (E)xit: ',iser,1,pasnumser(emis,gind)-1,erro)
        if(erro.eq.0) then
          call inpnum('Enter final serie to search or (E)xit: ',fser,iser,pasnumser(emis,gind)-1,erro)
          if(erro.lt.0) goto 10
        elseif (erro.eq.-4) then
          iser = 1
...
!==============================================================================
        ...
        write(fcod,900) strtyp(gind),week,year,pasemis(emis,gind)
        call ropen(fcod,rlun,erro)
        if(erro.ne.0) then
          type*,iam(),'Error opening cod file: ' // fcod
          goto 10
        endif
!===============================================================================
!      READING TPF FILE
            ...
            do mfra = ifra,ffra
              ...
              if(pasfdb.err.ne.ioe_noerr) then
                ...
                if(sbil.eq.pasrec.stat.or.sbil.gt.pbilmax) then
                  write(rlun,910) mnum,mser,mfra-1
...
!       Formats
!===============================================================================
900     format('COD',A1,I2.2,I2.2,'_',I4.4,'.TST')
910     format(I5.5,' ',I2.2,' ',I2.2,' 00000 V')
        end

C GUICMD_013.FOR
C
C V02 15-JAN-2011 FJG Lotto2 Batch: Lucky Number entry
C V01 01-FEB-2001 HXK INITIAL RELEASE
C
C LOTTO WINNING NUMBERS ENTRY COMMAND
C
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C This item is the property of GTECH Corporation, Providence, Rhode
C Island, and contains confidential and trade secret information. It
C may not be transferred from the custody or control of GTECH except
C as authorized in writing by an officer of GTECH. Neither this item
C nor the information it contains may be used, transferred,
C reproduced, published, or disclosed, in whole or in part, and
C directly or indirectly, except as expressly authorized by an
C officer of GTECH, pursuant to written agreement.
C
C Copyright 2001 GTECH Corporation. All rights reserved.
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
C This subroutine returns GUI FUNCTION.
C
C Input parameters:
C	NONE               
C
C Output parameters:
C
C	BYTE		OUTBUF(*)    OUTPUT MESSAGE
C	INTEGER*4	MES_LEN	     MESSAGE LENGTH
C	INTEGER*4	RET_CODE:
C		0		-  no error, message accepted;
C		value >= 11	-  error number to be sent to Client.
C
C
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE GUICMD_013(OUTBUF,MES_LEN,RET_CODE)
	IMPLICIT NONE
C
	INCLUDE 'INCLIB:SYSPARAM.DEF'
	INCLUDE 'INCLIB:SYSEXTRN.DEF'
C
        INCLUDE 'INCLIB:GUIMPRM.DEF'
        INCLUDE 'INCLIB:GUIARGS.DEF'
        INCLUDE 'INCLIB:RESCOM.DEF'
        INCLUDE 'INCLIB:LTOCOM.DEF'
        INCLUDE 'INCLIB:CONCOM.DEF'	
        INCLUDE 'INCLIB:GUIFIL.DEF'	        
C	
C
C
	BYTE		OUTBUF(*)
	INTEGER*4	MES_LEN,RET_CODE
C
	INTEGER*4 ST
	INTEGER*4 NUM_COLS, NUM_ROWS
	INTEGER*4 PAR,GIND,DRW
	INTEGER*4 N_WINNING_NUMS,N_WINNING_BONS
	INTEGER*4 N_WINNING_LUCK
	INTEGER*4 BUF(CDLEN)
	INTEGER*4 STATUS
	CHARACTER*40 STATUS_STR
	INTEGER*4 IND
	INTEGER*4 I
	INTEGER*4 BDROFF
	INTEGER*4 OFF
	INTEGER*4 GNUM
	
C
	INTEGER*4 PARAM_CNT 
	PARAMETER(PARAM_CNT=5+LTGNBR+LTGBON)
C
C
	RET_CODE = 0
	STATUS   = 0
	STATUS_STR = ' '
C
C
	CALL GUI_GETPARAMS(OUTBUF,ST)
	IF(ST.NE.0) THEN
	   RET_CODE = 11
	   RETURN
	ENDIF

	PAR = GUI_ARGVAL(1)
	IF(PAR.LT.1 .OR. PAR.GT.2) THEN
	    RET_CODE = 11
	    RETURN
	ENDIF

	GIND = GUI_ARGVAL(2)
	IF(GIND.LT.1 .OR. GIND.GT.MAXIND) THEN
	    STATUS = 2
	    WRITE(STATUS_STR,902) 1,MAXIND
	    GOTO 100
	ENDIF

	GNUM = GTNTAB(TLTO,GIND)

	DRW = GUI_ARGVAL(3)
	IF(LTODRW(GIND).EQ.0) THEN
          IF(DRW.LE.0 .OR. DRW.GT.DAYHDR(GNUM)) THEN
            STATUS = 3
            WRITE(STATUS_STR,903) DRW
            GOTO 100
          ENDIF
	ELSE
          IF(DRW.LE.0 .OR. DRW.GT.LTODRW(GIND)) THEN
            STATUS = 3
            WRITE(STATUS_STR,903) DRW
            GOTO 100
          ENDIF
        ENDIF

	IF(PAR.EQ.1) THEN
	 IF(DRW.EQ.LTODRW(GIND)) THEN	
	   CALL GAMLOG(TLTO,GIND,DLTREC,LTOBLK)
	 ELSE	     	
	   CALL READW(GAMFDB(1,GNUM),DRW,DLTREC,ST)
  	   IF(ST.NE.0) THEN
	     CALL OPS('Failed to read '//CGFNAMES(GNUM),ST,DRW)
             WRITE(STATUS_STR,914) DRW
	     RET_CODE = 14
	     GOTO 100
   	   ENDIF
         ENDIF    	 
	ENDIF

	N_WINNING_NUMS = GUI_ARGVAL(4)

	IF(N_WINNING_NUMS.LT.1 .OR. 
     *     N_WINNING_NUMS.GT.DLTNUM) THEN
	    STATUS = 4
	    WRITE(STATUS_STR,904) N_WINNING_NUMS
	    GOTO 100
	ENDIF

	N_WINNING_BONS = GUI_ARGVAL(5)
	IF(N_WINNING_BONS.NE.DLTBFL) THEN
          STATUS = 5
	  WRITE(STATUS_STR,905) N_WINNING_BONS
          GOTO 100
	ENDIF
	IND = 5

	BDROFF = 1
	
	IF(PAR.EQ.1 .AND. DLTSTS.EQ.GAMBFD) THEN
	    DO I=1,N_WINNING_NUMS
	       IND = IND + 1
	       IF(GUI_ARGVAL(IND).GE.1 .AND. GUI_ARGVAL(IND).LE.DLTMAX) THEN
	           DLTWIN(I,BDROFF) = GUI_ARGVAL(IND)
	       ELSE
	           STATUS = 6
	           WRITE(STATUS_STR,906) DLTMAX
	           GOTO 100
	       ENDIF
	    ENDDO
	    DO I=1,N_WINNING_BONS
	       IND = IND + 1
	       IF(GUI_ARGVAL(IND).GE.1 .AND. GUI_ARGVAL(IND).LE.DLTMAX) THEN
	           DLTBNM(I,BDROFF) = GUI_ARGVAL(IND)
	       ELSE
	           STATUS = 6
	           WRITE(STATUS_STR,906) DLTMAX
	           GOTO 100
	       ENDIF
	    ENDDO
	    IND = IND + 1	    
	    N_WINNING_LUCK = GUI_ARGVAL(IND)
            DLTLNM(BDROFF) = N_WINNING_LUCK
	    CALL LTOCHK(BDROFF,ST)
	    IF(ST.NE.0) THEN
	       STATUS = 7
	       WRITE(STATUS_STR,907)
	       GOTO 100
	    ENDIF
	    DLTSTS = GAMEN1
	    CUR_GIND = GIND
	    CUR_GTYP = TLTO
	    CUR_DRAW = DRW
	    WRITE(STATUS_STR,9011) GIND, DRW	    
	ELSEIF(PAR.EQ.2 .AND. DLTSTS.EQ.GAMEN1) THEN
	    DO I=1,N_WINNING_NUMS
	       IND = IND + 1
	       IF(GUI_ARGVAL(IND).GE.1 .AND. GUI_ARGVAL(IND).LE.DLTMAX) THEN
	           DLTHLD(I,BDROFF) = GUI_ARGVAL(IND)
	       ELSE
	           STATUS = 8
	           WRITE(STATUS_STR,908) DLTMAX
	           GOTO 100
	       ENDIF
	    ENDDO
	    DO I=1,N_WINNING_BONS
	       IND = IND + 1
	       IF(GUI_ARGVAL(IND).GE.1 .AND. GUI_ARGVAL(IND).LE.DLTMAX) THEN
	           DLTBHL(I,BDROFF) = GUI_ARGVAL(IND)
	       ELSE
	           STATUS = 8
	           WRITE(STATUS_STR,908) DLTMAX
	           GOTO 100
	       ENDIF
	    ENDDO
	    IND = IND + 1
	    N_WINNING_LUCK = GUI_ARGVAL(IND)	
            DLTLHL(BDROFF) = N_WINNING_LUCK	        
	    CALL VER_LTOCHK(BDROFF,ST)
	    IF(ST.NE.0) THEN
	       STATUS = 9
	       WRITE(STATUS_STR,909)
	       GOTO 100
	    ENDIF
	    DLTSTS = GAMENV
            DO I=1,DLTNUM
                IF(DLTWIN(I,BDROFF).NE.DLTHLD(I,BDROFF)) THEN
                    DLTSTS=GAMBFD
                    STATUS = 10
	            WRITE(STATUS_STR,910)
	            GOTO 100
                ENDIF
            END DO
            DO I=1,DLTBFL
                IF(DLTBNM(I,BDROFF).NE.DLTBHL(I,BDROFF)) THEN
                    DLTSTS=GAMBFD
                    STATUS = 11
	            WRITE(STATUS_STR,911)
	            GOTO 100
                ENDIF
            END DO

            IF(DLTLHL(BDROFF).NE.DLTLNM(BDROFF)) THEN
              DLTSTS=GAMBFD
              STATUS = 16
              WRITE(STATUS_STR,916)
              GOTO 100
            ENDIF
            	
            CALL MLTWIN(GNUM,DRW,ST)
            IF(ST.NE.0) THEN
               DLTSTS=GAMBFD
	       RET_CODE = 11
	      RETURN
	    ENDIF

            IF(DRW.EQ.LTODRW(GIND)) THEN
              CALL FASTSET(0,BUF,CDLEN)
              BUF(1) = 2
              BUF(2) = BDROFF
              BUF(3) = TCLTO
              BUF(6) = 'GUI '
              BUF(8) = GIND
              OFF = 0
              DO I=1,DLTNUM
                 CALL ISBYTE(DLTWIN(I,BDROFF),BUF(9),OFF)
                 OFF=OFF+1
              ENDDO
              DO I=1,DLTBFL
                 CALL ISBYTE(DLTBNM(I,BDROFF),BUF(9),OFF)
                 OFF=OFF+1
              ENDDO
              CALL ISBYTE(DLTLNM(BDROFF),BUF(9),OFF)     !V02
              OFF=OFF+1                                   !V02
                            
              CALL QUECMD(BUF,ST)
  	      IF(ST.NE.0) THEN
                 DLTSTS=GAMBFD
	         RET_CODE = 11
	         RETURN
	      ENDIF

              CALL FASTSET(0,BUF,CDLEN)
              BUF(1) = 1
              BUF(2) = DLTSTS
              BUF(3) = TCLTO
              BUF(6) = 'GUI '
              BUF(8) = GIND
              CALL QUECMD(BUF,ST)
  	      IF(ST.NE.0) THEN
                 DLTSTS=GAMBFD
	         RET_CODE = 11
	         RETURN
	      ENDIF
	    ELSE	    	
              CALL WRITEW(GAMFDB(1,GNUM),DRW,DLTREC,ST)
              IF(ST.NE.0) THEN
                STATUS = 15
         	CALL OPS('Failed to write '//CGFNAMES(GNUM),ST,DRW)
                WRITE(STATUS_STR,915) DRW
                GOTO 100
              ENDIF 
      	      CALL LORESULT(GIND,DRW)	    	
	    ENDIF	    
	    WRITE(STATUS_STR,9012) GIND, DRW	    	    
	ELSEIF(DLTSTS.EQ.GAMENV) THEN
	    STATUS = 12
	    WRITE(STATUS_STR,912) 
	    GOTO 100
	ELSE
	    STATUS = 13
	    WRITE(STATUS_STR,913) PAR,DLTSTS
	    GOTO 100
	ENDIF
C
C SEND DATA TO GUI
C
100	CONTINUE
	CALL GUIARG_INIT()
C
	NUM_COLS = 2
	NUM_ROWS = 1
	CALL GUIARG_NEXT_SET(OUTBUF,NUM_COLS)
C
C STATUS BACK
C
	CALL GUIARG_INT4(OUTBUF,STATUS)	
	CALL GUIARG_CHAR(OUTBUF,%REF(STATUS_STR),40)	
C
	CALL GUIARG_SET_MESLEN(MES_LEN)
C     
        CALL OPS('INFO: ' // STATUS_STR,STATUS,0)	
C
	RETURN
9011    FORMAT('Numbers entered  Lotto',I2,' Draw#',I4)	
9012    FORMAT('Numbers verified Lotto',I2,' Draw#',I4)	
902	FORMAT('Invalid game index: <',I8,' or >',I8)
903	FORMAT('Invalid Draw#: ',I8)
904	FORMAT('Invalid Number of winning numbers: ',I8)
905	FORMAT('Invalid Number of bonus numbers: ',I8)
906	FORMAT('Invalid number < 1 or > ',I2,' entered')
907	FORMAT('Duplicate numbers entered')
908	FORMAT('Invalid verification number < 1 or > ',I2,' entered')
909	FORMAT('Duplicate verification numbers entered')
910	FORMAT('Winning numbers verification error.')
911	FORMAT('Bonus numbers verification error.')
912	FORMAT('Winning numbers already entered and verified.')
913	FORMAT('Invalid - req mode:',I2,' game status:',I2)
914	FORMAT('Error reading draw file for draw:',I5)
915	FORMAT('Error writting draw file for draw:',I5)
916	FORMAT('Lucky number verification error.')
	END

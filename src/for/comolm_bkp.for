...
C                  CALL LIB$MOVC3(LEN, BPRO(BINPTAB+IND,BUF), TRABUF(TEUWBOARD))
                  CALL LIB$MOVC3(MESS_FROM_LEN, MESS_FROM_OLM, BPRO(BINPTAB,PROBUF))
C                  DO I=1, MESS_FROM_LEN    
C                        BPRO(BINPTAB+I-1,RBUF) = MESS_FROM_OLM(I-1) !BPRO(WRKTAB*4-3+I,RBUF)
C                  ENDDO
...
C----+------------------------------------------------------------------           
C variables for the vision programa for the version 0.3.0 will be commented until later version
C            IF(STATUS .NE. PAMS__BADARGLIST) THEN
C                  OLM_BADARGLIST_GET = OLM_BADARGLIST_GET + 1
C            ELSE IF(STATUS .NE. PAMS__BADPARAM) THEN
C                  OLM_BADPARAM_GET = OLM_BADPARAM_GET + 1
C            ELSE IF(STATUS .NE. PAMS__INSQUEFAIL) THEN
C                  OLM_INSQUEFAIL_GET = OLM_INSQUEFAIL_GET + 1
C            ELSE IF(STATUS .NE. PAMS__NETERROR) THEN
C                  OLM_NETERROR_GET = OLM_NETERROR_GET + 1
C                  OLM_TOTERRGET = OLM_TOTERRGET + 1
C            ELSE IF(STATUS .NE. PAMS__NOMEMORY) THEN
C                  OLM_NOMEMORY_GET = OLM_NOMEMORY_GET + 1
C            ELSE 
C                  OLM_TOTERRGET = OLM_TOTERRGET + 1
C            END IF 
C----+------------------------------------------------------------------

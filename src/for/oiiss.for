...
C=======OPTIONS /CHECK=NOOVERFLOW
        SUBROUTINE OIISS(TRABUF,OUTTAB,OUTLEN)
        IMPLICIT NONE
C
...
C
          OUTTAB(IND+0) = 0
          IF(TRABUF(TIRES+I).NE.INOER) OUTTAB(IND+0) = 1
          IND=IND+1
C
C INSTANT RESULT CODE TWO
C
          OUTTAB(IND+0) = 0
          IF(TRABUF(TIRES+I).NE.INOER) OUTTAB(IND+0) = TRABUF(TIRES+I)
          IND=IND+1
C
100     CONTINUE
C

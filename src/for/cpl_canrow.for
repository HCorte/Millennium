C
C SUBROUTINE CPL_CANROW
C $Log:   GXAFXT:[GOLS]CPL_CANROW.FOV  $
C  
...
C---- Chack to see if event is cancelled.
	...
	IF (N .EQ. 1) THEN
	  IF (CPROWSTS(ROW,N) .EQ. GAMCAN .OR. 
     *	      CPROWSTS(ROW,N) .EQ. GAMREF) THEN
	    WRITE(5,901) IAM(),ROW,(DCPNMS(K,ROW),K=1,3)
	    RETURN
	  END IF
	ELSE IF (N .EQ. 2) THEN
	  IF (CPROWSTS(ROW,N) .EQ. GAMCAN) THEN
	    WRITE(5,901) IAM(),ROW,(DCPNMS(K,ROW),K=1,3)
	    RETURN
	  END IF
	END IF
...
C---- Check if row active.
	...
	IF(CPROWSTS(ROW,N).EQ.GAMNUL) THEN
...
C---- Cancel row.
	...
	WRITE(5,902) IAM(),ROW,(DCPNMS(K,ROW),K=1,3)

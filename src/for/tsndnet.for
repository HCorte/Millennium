C
C SUBROUTINE TSNDNET
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]TSNDNET.FOV                                  $
C  $Date::   17 Apr 1996 15:38:30                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C
C     TSNDNET(BUF,WAY);SEND BUFFER TO NETLOG TOP OF THE QUEUE !!!
...
C
     ...
     *	    .OR.TOMASTER.NE.0) THEN
	   CALL ATL(BUF,NETFIL(1,WAY),ST)
	ELSE
	   CALL ATL(BUF,NETEXEC(1,WAY),ST)           !ADD BUFFER TO LIST
	ENDIF
C**	CALL QUEUE(8HNETLOG  ,BUF,ST)
	...
	1	ERROR_TEXT,,)
	    CALL OPS(CHAR(7)//ERROR_TEXT//CHAR(7),ST,8)
	ENDIF
C
	RETURN
	END

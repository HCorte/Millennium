C
C SUBROUTINE X2GRPDEL
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2GRPDEL.FOV                                 $
C  $Date::   17 Apr 1996 16:19:46                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
C This program will mark a record in the relay group file
...
50	CONTINUE
	...
	DO 100 I=1,LINES
	  OFFSET=(I-1)*2+1
C
C NOTE: OFFSET 1 IS HARDCODED TO ALLOW THE CHAR FIELDS
...
C
	IF(ANS.EQ.1) THEN
	  ...
	  X2XGRP_REC(1)=0
CV03	  CALL BSET(X2XGRP_BITMAP,1)
	  CALL X2BSET(X2XGRP_BITMAP,0,XGRP,REC)				!V03
	  UPDATE=.TRUE.
	ENDIF
C
...
C
	IF(UPDATE) THEN
	  ...
	  IF(ST.NE.0) THEN
	    CALL OS32ER(5,X2FILNAM(XGRP),'WRITEW',ST,1)
	    CALL GPAUSE
	  ENDIF
	  WRITE(5,9080) REC

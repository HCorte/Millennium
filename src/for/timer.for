...
C
	IF(LOGLOP.GE.P(LOGTIM)) THEN
	  ...
	  IF(P(TAPESW).NE.0) CALL RELSE(TSKNAM(TAP),ST)
	  IF(ACTTSK(RPC).NE.0) CALL RELSE(TSKNAM(RPC),ST)
	  IF(REPVQUE(2)-REPVQUE(3).NE.0) CALL RELSE(TSKNAM(VAL),ST)
C
C CHECK FOR PASSIVE GAME (VALIDATIONS AND RETURN TICKETS)
C
	  IF  (REPQUEPAS(2,RQPASPRO)-REPQUEPAS(3,RQPASPRO).NE.0)
     *	      CALL RELSE(TSKNAM(PST),ST)
	  IF  (REPQUEPAS(2,RQPASVAL)-REPQUEPAS(3,RQPASVAL).NE.0)

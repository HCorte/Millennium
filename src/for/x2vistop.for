C
C SUBROUTINE X2VISTOP
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2VISTOP.FOV                                 $
C  $Date::   17 Apr 1996 16:41:16                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE X2VISTOP(SCRIDX,LEVEL,DUMMY,LEN,ERR)
	IMPLICIT NONE
C
...
C
	ERR=0
	X2FLDINF(XTOPIDX)=1
	X2SCRN(X2SCRN_KEYLEV3+LEVEL,SCRIDX)=XTOPIDX
C
	RETURN
	END

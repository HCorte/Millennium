...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      ENDIF
C        ISTAT = lib$spawn('@GXOLM:MESSAGEQCONNECTION')
C        CALL OPSTXT('End')

C        CALL OPS('ISTAT:',ISTAT,ISTAT)
C        CALL OPS('SS$_NORMAL:',SS$_NORMAL,SS$_NORMAL)
C        CALL OPS('SS$_ACCVIO:',SS$_ACCVIO,SS$_ACCVIO)
C        CALL OPS('SS$_DUPLNAM:',SS$_DUPLNAM,SS$_DUPLNAM)
C        IF(.NOT. ISTAT) CALL LIB$SIGNAL(%VAL(ISTAT))
C        STATUS = lib$get_logical("MILLCONNECT",MILLCON)
...
C         STATLOG = STR$TRIM(ERR_MSG,MILLCON(1:POS_AUX-1),SIZE_AUX)
     ...
     *                     OLMS_TOTOKYGET                                      !TOTAL # OF MESSAGES RECEIVED FROM OLIMPO SYSTEM
       ...
       WRITE(CLIN21,9503) OLMS_TOTERRPUT,                                      !TOTAL # OF ERRORS WHILE SENDING MESSAGES TO OLIMPO SYSTEM

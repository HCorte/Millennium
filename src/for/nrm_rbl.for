...
C
C **** NOTE THAT THIS LOCKS BOTH ENDS OF A LIST
C
C RBL ----------------------------------------------------------
C
C
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE RBL (LIST_MEMBER, LIST_ARRAY, STATUS)
	IMPLICIT NONE
C
...
C
	LIST_MEMBER = LIST_ARRAY( LIST_ARRAY(GLIST_BOT) )
C
C UPDATE LIST HEADER INFO
C
	LIST_ARRAY (GLIST_BOT)     = LIST_OFFSET
	LIST_ARRAY (GLIST_RBL_CNT) = LIST_ARRAY (GLIST_RBL_CNT) + 1
C
C

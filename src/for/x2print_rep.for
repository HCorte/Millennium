...
C
C  $Logfile::   GXAFXT:[GOLS]X2PRINT_REP.FOV                              $
C  $Date::   17 Apr 1996 16:26:36                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
9200	FORMAT ('X2TERMSTA',I2.2,'.REP')
C	FILENAME='X2TERMSTA'//'03'//'.REP'
	OPEN(6,FILE=FILENAME,RECL=132,
     *	       STATUS='NEW',     IOSTAT=ST)
	IF(ST.NE.0) THEN
	  CALL OS32ER(5,'X2TERMSTA.REP','OPEN',ST,0)
	  CALL GPAUSE
	ENDIF
C

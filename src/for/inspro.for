...
C----+------------------------------------------------------------------
        IF(BPRO(CHOLM_OLM,BUF) .EQ. 1) THEN
          ...
          TRABUF(TVOLMMIDL_IL)=PRO(MESSID_INT_OLM,BUF)
          TRABUF(TVOLMMIDH_IL)=BPRO(MESSID_OLM+4,BUF)
...
C----+------------------------------------------------------------------  
     ...
     *               SALPAS)
        ...
        ELSE IF (TRABUF(TITYP).EQ.IMNU) THEN !(IMNU=6 ) - INSTANT SUPPLY ORDER 0xD6 - supplyOrder
          CALL DIMNU(PRO(INPTAB,BUF),TRABUF,HPRO(INPLEN,BUF))
        ELSE IF (TRABUF(TITYP).EQ.IORD) THEN !(IORD=7 ) - INSTANT GAMES NAMES REQUEST 0xD7 - gameParameters

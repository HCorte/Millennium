C
C SUBROUTINE X2SPCDEL
C
C*************************** START X2X PVCS HEADER ****************************
C
C  $Logfile::   GXAFXT:[GOLS]X2SPCDEL.FOV                                 $
C  $Date::   17 Apr 1996 16:35:06                                         $
C  $Revision::   1.0                                                      $
C  $Author::   HXK                                                        $
C
...
C
	...
	INTEGER*4   LEN1,LEN2               !Char output
	INTEGER*4   K, INDX2, INDX1
...
C
50	CONTINUE
	LINES=X2XSPC_ENTRIES/2
	DO 100 I=1,LINES
	  OFFSET=(I-1)*2+1
C
C NOTE: OFFSETS 3-9 ARE HARDCODED TO ALLOW THE CHAR FIELDS
...
C
	IF(ANS.EQ.1) THEN
	  ...
	  X2XSPC_REC(1)=0
CV02	  CALL BSET(X2XSPC_BITMAP,1)
...
9000	FORMAT(/,T26,'GTECH Distributed Network',/,
     *	          T24,'Delete a Port from a Station',/)

C SUBROUTINE NEWFIL
C
C V04 04-JUL-2000 OXK -> NORMLIB
C V03 09 Jan 1994 HXK New call parameter, KEEP, which determines whether 
C			 file should be allocated.
C V02 24-OCT-1991 TKO CALL SUBCLRFIL to clear the file
...
C=======OPTIONS /CHECK=NOOVERFLOW
	SUBROUTINE NEWFIL_NCNTG(LUN, FILENAME, SECTORS, KEEP, ST)
	IMPLICIT NONE
C
	INTEGER*4	LUN
	CHARACTER	FILENAME*(*)
...
C
	IF(.NOT.KEEP) THEN
	   CALL CFILX_NCNTG(FILENAME, 0, 0, SECTORS*2, 0,0,0, ST)
	   IF(ST.NE.0) THEN
	     GOTO 9000
	   ENDIF
	ENDIF
